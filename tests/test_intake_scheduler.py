...
def _arm_manager(mgr, cap, data_root):
    ...
    mgr._policy = None
    mgr._adapter_fp = adapter_registry_fingerprint()
...
def test_managed_scheduler_temp_fixture_runs_real_pipeline(tmp_path, monkeypatch):
    import types
    db_conn, data_root = _setup_db(tmp_path, monkeypatch)
    watch = tmp_path / "watch"
    watch.mkdir(exist_ok=True)
    src = watch / "scheduler-live.md"
    src.write_text("# Scheduler\n\nEnough words for queryable intake from scheduler.", encoding="utf-8")
    ...
    try:
        ...
        while time.time() < deadline:
            run = db_conn.fetchone(
                "SELECT * FROM intake_runs WHERE trigger_kind = 'scheduler'"
            )
            if run and run["lifecycle_state"] in {"complete", "quarantined", "failed"}:
                break
            time.sleep(0.05)

        assert run is not None
        assert run["lifecycle_state"] == "complete"
        rev = db_conn.fetchone(
            "SELECT * FROM intake_source_revisions WHERE source_revision_id = ?",
            (run["source_revision_id"],),
        )
        assert rev["lifecycle_state"] == "complete"
        assert rev["adapter_registry_version"] == adapter_registry_fingerprint()
        ...
        assert cap["canon_eligible"] == 0
        raw = db_conn.fetchone(
            "SELECT COUNT(*) AS n FROM intake_raw_artifacts WHERE intake_capability_id = ?",
            (run["intake_capability_id"],),
        )
        norm = db_conn.fetchone(
            "SELECT COUNT(*) AS n FROM intake_normalized_artifacts n "
            "JOIN intake_raw_artifacts r ON r.raw_artifact_id = n.raw_artifact_id "
            "WHERE r.intake_capability_id = ?",
            (run["intake_capability_id"],),
        )
        assert raw["n"] == 1
        assert norm["n"] == 1
