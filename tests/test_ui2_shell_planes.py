...
- fold.js uses visiblePlanes, no scope prop, Bag of Holding breadcrumb
...
class TestShellSimplification:
    ...
    def test_library_chip_present(self):
        assert "Library: Bag of Holding" in _src(SHELL_JS)
...
class TestAppStateCleanup:
    ...
    def test_modal_dialog_uses_library_not_scope(self):
        ...
        assert '"Bag of Holding"' in src or "'Bag of Holding'" in src
...
class TestFoldFilter:
    ...
    def test_bag_of_holding_breadcrumb(self):
        assert "Bag of Holding" in _src(FOLD_JS)
