...
def test_upload_accept_lists_include_expanded_safe_types():
    index = (ROOT / "app" / "ui" / "index.html").read_text(encoding="utf-8")
    app_js = (ROOT / "app" / "ui" / "app.js").read_text(encoding="utf-8")
    for ext in [".mdx", ".toml", ".env.example", ".ipynb", ".docx"]:
        assert ext in index
        assert ext in app_js
    assert "supported.some(ext => lower.endsWith(ext))" in app_js
