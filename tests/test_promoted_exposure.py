...
def test_exposure_matrix_default_off_everywhere(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch) as (client, dbc, lib, src, data_root):
        ...
        fold_scoped = client.get("/api/fold/library", params={"library_id": "all"})
        assert fold_scoped.status_code == 200
        assert doc_id not in {d.get("doc_id") for d in fold_scoped.json().get("docs", [])}
        ...
        g_scoped = client.get("/api/graph/projection", params={"mode": "web", "library_id": "all"})
        if g_scoped.status_code == 200:
            assert doc_id not in str(g_scoped.json())
