...
Governance invariants checked:
  ...
  - unsupported axis (domain/batch) -> HTTP 400, never 500
...
class TestDomainClusterRoute:
    ...
    def test_domain_cluster_has_zero_members(self, tmp_path, monkeypatch):
        with _client(tmp_path, monkeypatch) as (client, db, _):
            _insert_doc(db, "p1", project="boh")
            body = client.get("/api/fold/cluster/domain/clinical").json()
            assert body["aggregation"]["inputs_count"] == 0

    def test_domain_cluster_has_unresolvable_unknown(self, tmp_path, monkeypatch):
        with _client(tmp_path, monkeypatch) as (client, db, _):
            body = client.get("/api/fold/cluster/domain/clinical").json()
            fields = [u["field"] for u in body["unknowns"]]
            assert "domain_membership_unresolvable" in fields

    def test_domain_cluster_diagnostic_only(self, tmp_path, monkeypatch):
        with _client(tmp_path, monkeypatch) as (client, db, _):
            body = client.get("/api/fold/cluster/domain/clinical").json()
    ...
    def test_domain_corpus_empty_registry_has_unresolvable_unknown(self, tmp_path, monkeypatch):
        with _client(tmp_path, monkeypatch) as (client, db, _):
    ...
    def test_domain_corpus_with_registered_domain_each_cluster_marked_degraded(self, tmp_path, monkeypatch):
        with _client(tmp_path, monkeypatch) as (client, db, _):
            _insert_lattice_domain(db, "clinical")
            body = client.get("/api/fold/cluster/domain/clinical").json()
            fields = [u["field"] for u in body["unknowns"]]
