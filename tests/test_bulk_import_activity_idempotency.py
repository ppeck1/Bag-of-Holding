...
def test_changed_upload_with_same_name_is_not_skipped(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch) as (client, db, _library, _autoindex):
        first = client.post(
            "/api/input/upload",
            data={"target_folder": "imports/activity"},
        ...
        changed = client.post(
            "/api/input/upload",
            data={"target_folder": "imports/activity"},
            files=[("files", ("doc.md", _plain_md("Browser Doc Revised"), "text/markdown"))],
            headers=_auth(),
        )
        assert changed.status_code == 200
        item = changed.json()["saved"][0]
        assert item["skipped"] is False
        assert item["action"] == "imported"
        assert item["path"] == "imports/activity/doc-2.md"

        paths = db.fetchall("SELECT path FROM docs ORDER BY path")
        assert [row["path"] for row in paths] == [
            "imports/activity/doc-2.md",
            "imports/activity/doc.md",
        ]
