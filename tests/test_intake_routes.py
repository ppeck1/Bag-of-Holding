...
def test_run_notebook_extracts_queryable_text(intake_client, tmp_path):
    client, db_conn, data_root = intake_client
    src = _make_source(
        tmp_path,
        "analysis.ipynb",
        json.dumps({
            "cells": [
                {"cell_type": "markdown", "source": ["# Notebook\n", "governed notebook notes"]},
                {"cell_type": "code", "source": ["value = 42"], "outputs": [{"text": "DROP ME"}]},
            ]
        }),
    )
    resp = client.post(
        "/api/intake/run",
        json={"source_ref": src, "batch_id": "batch-ipynb"},
        headers=_auth(),
    )
    assert resp.status_code == 200
    data = resp.json()
    assert data["status"] == "complete"
    cap_id = data["intake_capability_id"]
    cap = db_conn.fetchone("SELECT queryable FROM intake_capabilities WHERE intake_capability_id = ?", (cap_id,))
    assert cap["queryable"] == 1
    raw = db_conn.fetchone("SELECT raw_artifact_id FROM intake_raw_artifacts WHERE intake_capability_id = ?", (cap_id,))
    norm = db_conn.fetchone("SELECT output_path FROM intake_normalized_artifacts WHERE raw_artifact_id = ?", (raw["raw_artifact_id"],))
    text = Path(data_root, norm["output_path"]).read_text(encoding="utf-8")
    assert "governed notebook notes" in text
    assert "value = 42" in text
    assert "DROP ME" not in text


def test_run_docx_extracts_queryable_text(intake_client, tmp_path):
    client, db_conn, data_root = intake_client
    src = _make_source(tmp_path, "document.docx", _minimal_docx_bytes("Governed DOCX extracted words here."))
    resp = client.post(
        "/api/intake/run",
        json={"source_ref": src, "batch_id": "batch-docx"},
        headers=_auth(),
    )
    assert resp.status_code == 200
    data = resp.json()
    ...
    norm = db_conn.fetchone("SELECT output_path FROM intake_normalized_artifacts WHERE raw_artifact_id = ?", (raw["raw_artifact_id"],))
    text = Path(data_root, norm["output_path"]).read_text(encoding="utf-8")
    assert "Governed DOCX extracted words" in text
