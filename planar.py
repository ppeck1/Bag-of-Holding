"""planar.py: Planar math (Plane/Field/Node) storage and alignment scoring."""
...
def store_fact(
    ...
    """Validate and store a planar fact."""
...
def planar_alignment_score(plane_scopes: list[str], query_plane: Optional[str] = None) -> float:
    ...
    Score based on active facts in those planes: weighted by q*c, direction, recency.
    ...
    for row in rows:
        weight = row["q"] * row["c"]
        direction = row["d"]  # -1, 0, +1
        # Normalize direction contribution: -1 → 0, 0 → 0.5, +1 → 1
        normalized = (direction + 1) / 2.0
