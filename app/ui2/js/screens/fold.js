...
import { Badge, Button, Icon, Segmented, Accordion, EmptyState, MarkerPip, Tooltip, WhyCurrent } from "../primitives.js";
...
const VIEW_STORE = "boh_fold_view_v1";
const PLANE_KEYS = Object.keys(PLANES);
...
const STATE_LANE = { conflict: 0.14, expired: 0.3, stale: 0.46, unknown: 0.64, current: 0.82 };
const PROJECTION_META = {
  web: {
    lead: "topology",
    axes: "graph X / graph Y",
    note: "Graph projection coordinates from /api/graph/projection.",
  },
  risk: {
    lead: "risk x gap",
    axes: "X risk / Y readiness gap",
    note: "Risk is composite drift, conflict, and projection-loss pressure when available.",
  },
  authority: {
    lead: "tier lanes",
    axes: "X authority tier / Y fold state",
    note: "Tier is bucketed from authority pressure.",
  },
  currentness: {
    lead: "auth x fresh",
    axes: "X authority / Y freshness",
    note: "Batch fold pressures from /api/fold/library.",
  },
  evidence: {
    lead: "plane lanes",
    axes: "X plane / Y evidence kind",
    note: "Plane and kind are normalized from graph fields.",
  },
  timeline: {
    lead: "time lanes",
    axes: "X validity time / Y evidence kind",
    note: "Missing dates fall back to fold pressure position.",
  },
};
const LABEL_MODES = [
...
function foldAxisLabel(axis) { return axis === "domain" ? "Domain" : "Project"; }
function jitterPoint(p, nd, sx = 18, sy = 16) {
  if (nd.kind === "cluster") return p;
  return {
    x: clamp(p.x + (hash01(nd.id + ":x") - 0.5) * sx, PAD.l, LW - PAD.r),
    y: clamp(p.y + (hash01(nd.id + ":y") - 0.5) * sy, PAD.t, LH - PAD.b),
  };
}

function posFor(nd, proj) {
  const w = LW - PAD.l - PAD.r, h = LH - PAD.t - PAD.b;
  if (proj === "currentness") return { x: PAD.l + nd.authP * w, y: PAD.t + (1 - nd.freshP) * h };
  if (proj === "timeline") {
    const t = nd.time_norm ?? nd.time ?? 0.5;
    const lane = LANE[nd.evKind] ?? 0.5;
    return { x: PAD.l + t * w, y: PAD.t + lane * h };
  }
  if (proj === "risk") {
    return jitterPoint({ x: PAD.l + (nd.risk ?? 0.3) * w, y: PAD.t + (1 - (nd.canon ?? 0.4)) * h }, nd, 22, 20);
  }
  if (proj === "authority") {
    return jitterPoint({ x: PAD.l + (authTier(nd) / 3) * w, y: PAD.t + (STATE_LANE[nd.currentness] ?? 0.64) * h }, nd, 20, 14);
  }
  if (proj === "evidence") {
    const idx = PLANE_KEYS.indexOf(normalizePlaneKey(nd.plane));
    const planeX = idx >= 0 ? idx / Math.max(1, PLANE_KEYS.length - 1) : 0.5;
    const lane = nd.kind === "cluster" ? LANE.cluster : (LANE[nd.evKind] ?? 0.28);
    return jitterPoint({ x: PAD.l + planeX * w, y: PAD.t + lane * h }, nd, 18, 16);
  }
  return { x: PAD.l + nd.bx * w, y: PAD.t + nd.by * h };
}
...
function loadViewPrefs() {
  const fallback = { labelMode: "auto", edgeMode: "smart", clusterAxis: "project", focusCluster: false, toolkitOpen: true };
  try {
    const raw = JSON.parse(sessionStorage.getItem(VIEW_STORE) || "null") || {};
    return {
      labelMode: LABEL_MODES.some(o => o.value === raw.labelMode) ? raw.labelMode : fallback.labelMode,
      edgeMode: EDGE_MODES.some(o => o.value === raw.edgeMode) ? raw.edgeMode : fallback.edgeMode,
      clusterAxis: FOLD_AXES.some(o => o.value === raw.clusterAxis) ? raw.clusterAxis : fallback.clusterAxis,
      focusCluster: !!raw.focusCluster,
      toolkitOpen: raw.toolkitOpen == null ? fallback.toolkitOpen : !!raw.toolkitOpen,
    };
  } catch (_) {
    return fallback;
  }
...
function saveViewPrefs() {
  try {
    sessionStorage.setItem(VIEW_STORE, JSON.stringify({
      labelMode: st.labelMode,
      edgeMode: st.edgeMode,
      clusterAxis: st.clusterAxis,
      focusCluster: st.focusCluster,
      toolkitOpen: st.toolkitOpen,
    }));
  } catch (_) {}
}
...
// ── exported component ────────────────────────────────────────────────────────
export function FoldWorkspace({ visiblePlanes, data, onToast, activeLibrary, activeLibraryId }) {
  const D = (data && Array.isArray(data.nodes)) ? data : { nodes: [], edges: [] };
  const nodeById = {}; D.nodes.forEach(n => (nodeById[n.id] = n));

  function nodeClusterIds(nd, axis = st.clusterAxis) {
    if (!nd || nd.kind === "cluster") return [];
    const fromMap = nd.clusters && nd.clusters[axis];
    if (Array.isArray(fromMap)) return fromMap;
    if (fromMap) return [fromMap];
    if (axis === "project" && nd.cluster) return [nd.cluster];
    return [];
  }
  function activeClusterId() {
    const nd = st.sel ? nodeById[st.sel] : null;
    if (!nd) return null;
    if (nd.kind === "cluster") return nd.axis === st.clusterAxis ? nd.id : null;
    return nodeClusterIds(nd)[0] || null;
  }
  function planeVisible(nd) {
    const key = normalizePlaneKey(nd.plane);
    return !key || !visiblePlanes || visiblePlanes.includes(key);
  }
  function clusterMembers(clusterId) {
    const cluster = nodeById[clusterId];
    const axis = (cluster && cluster.axis) || st.clusterAxis;
    return D.nodes.filter(n => n.kind !== "cluster" && nodeClusterIds(n, axis).includes(clusterId) && planeVisible(n));
  }
  function displayPosFor(nd) {
    return posFor(nd, st.proj);
  }

  // which nodes are visible (plane filter + cluster logic + hidden/expanded)
  function visNodes() {
    const focusId = st.focusCluster ? activeClusterId() : null;
    return D.nodes.filter(n => {
      if (n.kind === "cluster") {
        if (n.axis !== st.clusterAxis) return false;
        if (focusId && n.id !== focusId) return false;
        // cluster visible iff at least one member's plane passes the filter
        return clusterMembers(n.id).length > 0;
      }
      if (!planeVisible(n)) return false;
      const clusterIds = nodeClusterIds(n);
      if (focusId) return clusterIds.includes(focusId);
      if (!clusterIds.length) return true;
      return clusterIds.some(cid => st.expanded.has(cid));
    });
  }
  ...
  function visEdges() {
    ...
    const edges = D.edges.filter(e => vis.has(e.from) && vis.has(e.to));
    const clusterIds = visNodes().filter(n => n.kind === "cluster").map(n => n.id);
    clusterIds.forEach(cid => {
      const shouldUnfold = st.expanded.has(cid) || (st.focusCluster && activeClusterId() === cid);
      if (!shouldUnfold) return;
      clusterMembers(cid).forEach(m => {
        if (vis.has(m.id)) edges.push({ from: cid, to: m.id, type: "wrapped_as", member: true });
      });
    });
    return edges;
  }
  ...
  function showEdge(e) {
    if (st.edgeMode === "none") return false;
    if (st.edgeMode === "all") return true;
    if (!st.sel && !st.hover) return st.edgeMode === "selected" ? false : st.cam.z < 1.6; // default: show if not overloaded
    const active = st.sel || st.hover;
    if (st.edgeMode === "selected") return e.from === active || e.to === active;
  ...
  function loadPacket(id) {
    ...
    const fetcher = (nd && nd.kind === "cluster")
      ? fetchFoldCluster(nd.axis, nd.value, activeLibraryId)
      : fetchFoldNode(id, activeLibraryId);
    fetcher.then(pk => {
  ...
  function fitSel() {
    ...
    const pts = visNodes().filter(n => hood.has(n.id)).map(n => displayPosFor(n));
  ...
  function buildEdge(e) {
    ...
    const a = nodeById[e.from] ? displayPosFor(nodeById[e.from]) : null;
    const b = nodeById[e.to]   ? displayPosFor(nodeById[e.to]) : null;
    if (!a || !b) return null;
    const active = st.sel || st.hover;
  ...
  function buildNode(nd) {
    ...
    if (showLabel(nd)) {
      if (nd.kind === "cluster") {
        const visCount = clusterMembers(nd.id).length;
        const baseLabel = `${foldAxisLabel(nd.axis)} · ${nd.label}`;
        const clusterLabel = visCount === nd.count ? `${baseLabel} · ${nd.count}` : `${baseLabel} · ${visCount}/${nd.count}`;
  ...
  function buildAxes() {
    ...
    if (st.proj === "risk") return hs("g", {},
      hs("line", { class: "axis-line", x1: x0, y1: y1, x2: x1, y2: y1 }),
      hs("line", { class: "axis-line", x1: x0, y1: y0, x2: x0, y2: y1 }),
      hs("line", { class: "axis-line ghost", x1: x0 + (x1 - x0) * 0.5, y1: y0, x2: x0 + (x1 - x0) * 0.5, y2: y1 }),
      hs("line", { class: "axis-line ghost", x1: x0, y1: y0 + (y1 - y0) * 0.5, x2: x1, y2: y0 + (y1 - y0) * 0.5 }),
      hs("text", { class: "axis-label", x: x1, y: y1 + 22, "text-anchor": "end" }, "risk pressure ->"),
      hs("text", { class: "axis-label", x: x0 - 10, y: y0 - 8, "text-anchor": "start" }, "readiness gap ->"),
      hs("text", { class: "quad-label", x: x1 - 12, y: y0 + 16, "text-anchor": "end" }, "urgent gap"),
      hs("text", { class: "quad-label", x: x0 + 8, y: y1 - 10 }, "low risk"));
    if (st.proj === "authority") return hs("g", {},
      hs("line", { class: "axis-line", x1: x0, y1: y1, x2: x1, y2: y1 }),
      hs("line", { class: "axis-line", x1: x0, y1: y0, x2: x0, y2: y1 }),
      ...[0, 1, 2, 3].map(t => hs("text", { class: "quad-label", x: x0 + (t / 3) * (x1 - x0), y: y1 + 22, "text-anchor": "middle" }, `tier ${t}`)),
      ...Object.entries(STATE_LANE).map(([k, v]) => hs("text", { class: "quad-label", x: x0 - 8, y: y0 + v * (y1 - y0) + 4, "text-anchor": "end" }, k)),
      hs("text", { class: "axis-label", x: x1, y: y1 + 40, "text-anchor": "end" }, "authority tier ->"),
      hs("text", { class: "axis-label", x: x0 - 10, y: y0 - 8, "text-anchor": "start" }, "fold state lanes"));
    if (st.proj === "evidence") return hs("g", {},
      hs("line", { class: "axis-line", x1: x0, y1: y1, x2: x1, y2: y1 }),
      ...PLANE_KEYS.map((k, i) => {
        const x = x0 + (i / Math.max(1, PLANE_KEYS.length - 1)) * (x1 - x0);
        return hs("text", { class: "quad-label", x, y: y1 + 20, "text-anchor": "middle" }, PLANES[k].glyph);
      }),
      ...(Object.entries(LANE).map(([k, v]) => hs("text", { class: "quad-label", x: x0 - 8, y: y0 + v * (y1 - y0) + 4, "text-anchor": "end" }, k))),
      hs("text", { class: "axis-label", x: x1, y: y1 + 38, "text-anchor": "end" }, "plane lanes ->"),
      hs("text", { class: "axis-label", x: x0 - 10, y: y0 - 8, "text-anchor": "start" }, "evidence kind lanes"));
  ...
  function repaintChrome() {
    if (R.foldCol) mount(R.foldCol, [buildBar(), buildCrumbs(), buildMainView()]);
  }
  ...
  function setViewPref(key, value, repaint = "all") {
    st[key] = value;
    saveViewPrefs();
    if (repaint === "light") {
      repaintEdges(); repaintNodes(); repaintLegend();
    } else {
      repaintAll();
    }
  }
  function setClusterAxis(axis) {
    if (!FOLD_AXES.some(o => o.value === axis) || st.clusterAxis === axis) return;
    const selected = st.sel ? nodeById[st.sel] : null;
    st.clusterAxis = axis;
    st.focusCluster = false;
    if (selected && selected.kind === "cluster" && selected.axis !== axis) {
      st.sel = null; st.packet = null; st.packetFor = null; st.packetLoading = false;
    }
    saveViewPrefs();
    repaintAll();
  }
  ...
  function buildToolkit() {
    ...
    const meta = PROJECTION_META[st.proj] || {};
    const head = h("button", {
      class: "fold-toolkit-head",
      "aria-expanded": st.toolkitOpen ? "true" : "false",
      onClick: () => setViewPref("toolkitOpen", !st.toolkitOpen),
    },
      h("span", { class: "t-micro" }, "View toolkit"),
      h("span", { class: "spacer" }),
      h("span", { class: "t-small muted" }, meta.axes || projDef.lead),
      Icon({ name: st.toolkitOpen ? "chevDown" : "chevRight", size: 13 }));
    if (!st.toolkitOpen) return h("div", { class: "fold-toolkit" }, head);
    return h("div", { class: "fold-toolkit" }, head,
      h("div", { class: "fold-toolkit-body" },
        h("div", { class: "ft-row" }, h("span", { class: "ft-key" }, "Labels"),
          Segmented({ sm: true, value: st.labelMode, onChange: v => setViewPref("labelMode", v), options: LABEL_MODES })),
        h("div", { class: "ft-row" }, h("span", { class: "ft-key" }, "Edges"),
          Segmented({ sm: true, value: st.edgeMode, onChange: v => setViewPref("edgeMode", v), options: EDGE_MODES })),
        h("div", { class: "ft-row" }, h("span", { class: "ft-key" }, "Folds"),
          Segmented({ sm: true, value: st.clusterAxis, onChange: setClusterAxis, options: FOLD_AXES })),
        h("div", { class: "ft-row wrap" }, h("span", { class: "ft-key" }, "Focus"),
          h("button", {
            class: `chip ${st.focusCluster ? "active" : ""}`.trim(),
            "aria-pressed": st.focusCluster ? "true" : "false",
            onClick: () => {
              const cid = activeClusterId();
              if (!cid) { onToast && onToast("Select a fold before using Cluster Focus.", "unknown"); return; }
              st.expanded.add(cid);
              setViewPref("focusCluster", !st.focusCluster);
            },
          }, "Cluster Focus"),
          Button({ variant: "secondary", className: "sm", glyph: "o", onClick: fitSel, children: ["Fit"] }),
          Button({ variant: "ghost", className: "sm", glyph: "x", onClick: () => { st.focusCluster = false; saveViewPrefs(); resetCam(); repaintAll(); }, children: ["Reset"] })),
        h("div", { class: "ft-note t-small muted" }, `${meta.note || ""} ${data && data.inferred ? "Joined client-side from fold and graph endpoints." : ""}`.trim())));
  }
  ...
  function buildStage() {
    ...
    const meta = h("div", { class: "fold-meta" },
      h("span", { class: "chip is-static" }, (() => {
        ...
        return `${docCount} nodes · ${edgeCount} edges · ${foldAxisLabel(st.clusterAxis)} folds${planesInfo}`;
      })()),
      R.zoomPct);
    ...
    R.stageWrap = h("div", { class: "fold-stage", style: { position: "relative" } }, svg, R.legendWrap, buildToolkit(), meta, zoomControls, R.tooltip);
    return R.stageWrap;
  }
  ...
  function buildLegend() {
    ...
    const head = h("div", { class: "fl-head", onClick: () => { st.legendOpen = !st.legendOpen; repaintLegend(); } },
      ...
      h("span", { class: "fl-chan" }, `lead: ${meta.lead || projDef.lead}`),
    ...
    return h("div", { class: "fold-legend" }, head,
      h("div", { class: "fl-body" },
        h("div", { class: "fl-row" }, h("span", { class: "fl-key" }, "axes"), h("span", { class: "fl-chip" }, meta.axes || projDef.lead)),
        stateRow,
        ...
        keys,
        meta.note && h("div", { class: "fl-note" }, meta.note)));
  }
  ...
  function clusterDist(clusterId) {
    const axis = (nodeById[clusterId] && nodeById[clusterId].axis) || st.clusterAxis;
    const members = D.nodes.filter(n => nodeClusterIds(n, axis).includes(clusterId) && n.kind !== "cluster");
  ...
  function buildInspector() {
    ...
    if (nd.kind === "cluster") {
      ...
      return h("div", { class: "inspector-body" },
        h("div", null,
          ...
          h("div", { class: "t-small muted" }, `${foldAxisLabel(nd.axis)} fold · ${aggMemberCount} members`)),
    ...
    return h("div", { class: "inspector-body" },
      ...
      AssociatedFilePreview({
        docId: nd.id,
        path: (pk && pk.local_state && pk.local_state.path) || nd.path,
        title: nd.label || nd.id,
      }));
  }
  ...
  // ── list view ────────────────────────────────────────────────────────────
  function attentionReasons(nd) {
    const reasons = [];
    if (nd.currentness === "conflict") reasons.push("conflicted");
    if (nd.currentness === "stale") reasons.push("stale");
    if (nd.currentness === "expired") reasons.push("expired");
    if (nd.currentness === "unknown") reasons.push("unknown");
    if ((nd.risk ?? 0) >= 0.65) reasons.push("high risk");
    if ((nd.authP ?? 0) < 0.3) reasons.push("low authority");
    (nd.markers || []).forEach(m => reasons.push(nsMeta(m).label || m));
    return reasons;
  }
  ...
  function buildActionList() {
    const rows = visNodes()
      ...
      .map(n => ({ nd: n, reasons: attentionReasons(n) }))
      .filter(r => r.reasons.length)
      .sort((a, b) => (b.nd.risk ?? 0) - (a.nd.risk ?? 0) || String(a.nd.label).localeCompare(String(b.nd.label)));
    if (!rows.length) {
      return h("div", { class: "fold-list" },
        EmptyState({ glyph: "o", title: "No action nodes", desc: "Current filters do not expose conflicted, stale, unknown, review-marked, or high-risk nodes." }));
    }
    return h("div", { class: "fold-list" }, h("div", { class: "card", style: { overflow: "hidden" } },
      h("div", { class: "card-head" },
        h("span", { class: "t-subheading" }, "Action Queue"),
        h("span", { class: "spacer" }),
        h("span", { class: "t-small muted" }, `${rows.length} attention node${rows.length === 1 ? "" : "s"}`)),
      h("div", { style: { overflowX: "auto" } }, h("table", { class: "tbl" },
        h("thead", null, h("tr", null,
          h("th", { style: { width: "120px" } }, "State"),
          h("th", null, "Document"),
          h("th", { style: { width: "220px" } }, "Why"),
          h("th", { class: "num", style: { width: "80px" } }, "Risk"),
          h("th", { class: "num", style: { width: "90px" } }, "Authority"))),
        h("tbody", null, rows.map(({ nd, reasons }) => h("tr", {
          class: st.sel === nd.id ? "selected" : "", tabindex: "0",
          ...
          h("td", null, h("span", { class: "muted" }, reasons.slice(0, 4).join(", "))),
          h("td", { class: "num muted" }, (nd.risk ?? 0).toFixed(2)),
          h("td", { class: "num muted" }, (nd.authP ?? 0).toFixed(2)))))))));
  }
  ...
  function buildMainView() {
    if (st.view === "list") return buildList();
    if (st.view === "actions") return buildActionList();
    return buildStage();
  }
    ...
    return h("div", { class: "fold-bar" },
      h("div", { class: "proj-switch", role: "group", "aria-label": "Projection" },
        PROJECTIONS.map(p => h("button", { class: `proj-seg ${st.proj === p.id ? "active" : ""}`.trim(), onClick: () => { if (st.proj !== p.id) { st.proj = p.id; repaintAll(); } } },
          h("span", { class: "ps-name" }, p.name), h("span", { class: "ps-lead" }, (PROJECTION_META[p.id] || p).lead)))),
      ...
      h("span", { class: "spacer" }),
      Segmented({ sm: true, value: st.view, onChange: v => { if (st.view !== v) { st.view = v; repaintAll(); } }, options: [{ value: "canvas", label: "Canvas" }, { value: "list", label: "List" }, { value: "actions", label: "Actions" }] }),
      Button({ variant: "secondary", className: "sm", glyph: "⤢", onClick: fitSel, children: ["Fit"] }));
  }

  function buildCrumbs() {
    const sel = st.sel ? nodeById[st.sel] : null;
    const libraryName = (activeLibrary && activeLibrary.name) || "All libraries";
    return h("div", { class: "fold-crumbs" },
      h("span", { class: sel ? "crumb" : "crumb last", onClick: () => select(null) }, libraryName),
  ...
  R.foldCol = h("div", { class: "fold" }, buildBar(), buildCrumbs(), buildMainView());
