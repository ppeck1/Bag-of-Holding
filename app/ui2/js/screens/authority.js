...
export function AuthorityScreen({ onNavigate, onSelectIntegrityItem, onSelectLedgerEvent, onSelectGateResult, onSelectResidence }) {
  let root;
  ...
  function build() {
    root = h("div", { class: "content content-narrow" },
      ...
      _tab === "integrity" ? IntegrityTab(onSelectIntegrityItem) :
      _tab === "ledger"    ? LedgerTab(onSelectLedgerEvent) :
      _tab === "trace"     ? TraceTab(onSelectGateResult) :
                             ResidenceTab(onSelectResidence));
    return root;
  }
  return build();
}

function IntegrityTab(onSelectIntegrityItem) {
  const wrap = loading();
  api("/api/integrity/dashboard").then(d => {
    ...
    if (risk.length) out.appendChild(Card({ title: "Highest drift risk", bodyFlush: true, children:
      tbl(["Node","State","Drift","Valid until"], risk.map(r => ({
        ...
        id: r.node_id,
        type: "drift_risk",
        onClick: () => onSelectIntegrityItem && onSelectIntegrityItem({ ...r, id: r.node_id, type: "drift_risk" }),
        onKeydown: e => { if (e.key === "Enter") onSelectIntegrityItem && onSelectIntegrityItem({ ...r, id: r.node_id, type: "drift_risk" }); },
        cells: [
    ...
    if (auths.length) out.appendChild(Card({ title: "Authority violations", bodyFlush: true, children:
      tbl(["Target","Required","Attempted by","Time"], auths.map(a => ({
        ...
        id: a.target_id,
        type: "violation",
        onClick: () => onSelectIntegrityItem && onSelectIntegrityItem({ ...a, id: a.target_id, type: "violation" }),
        onKeydown: e => { if (e.key === "Enter") onSelectIntegrityItem && onSelectIntegrityItem({ ...a, id: a.target_id, type: "violation" }); },
        cells: [
...
function LedgerTab(onSelectLedgerEvent) {
  const wrap = loading();
    ...
    const out = h("div", { class: "col gap-4", style: { marginTop: "16px" } },
      Card({ title: `Promotion ledger (${promos.length})`, bodyFlush: true, children:
        promos.length ? tbl(["Target","From → To","Authority","Reason","When"], promos.map(p => ({
          key: p.target_id,
          id: p.target_id,
          onClick: () => onSelectLedgerEvent && onSelectLedgerEvent({ ...p, id: p.target_id }),
          onKeydown: e => { if (e.key === "Enter") onSelectLedgerEvent && onSelectLedgerEvent({ ...p, id: p.target_id }); },
          cells: [
      ...
      Card({ title: `Resolution log (${log.length})`, bodyFlush: true, children:
        log.length ? tbl(["When","Actor","Target","Required","Result"], log.map(e => ({
          key: e.target_id,
          id: e.target_id,
          onClick: () => onSelectLedgerEvent && onSelectLedgerEvent({ ...e, id: e.target_id }),
          onKeydown: e_ev => { if (e_ev.key === "Enter") onSelectLedgerEvent && onSelectLedgerEvent({ ...e, id: e.target_id }); },
          cells: [
...
function ResidenceTab(onSelectResidence) {
  const wrap = loading();
    ...
    if (rows.length) {
      out.appendChild(Card({ bodyFlush: true, children:
        tbl(["Original","Current","Status","Reason"], rows.map(r => ({
          key: r.id || r.residence_id,
          id: r.id || r.residence_id,
          onClick: () => onSelectResidence && onSelectResidence({ ...r, id: r.id || r.residence_id }),
          onKeydown: e => { if (e.key === "Enter") onSelectResidence && onSelectResidence({ ...r, id: r.id || r.residence_id }); },
          cells: [
