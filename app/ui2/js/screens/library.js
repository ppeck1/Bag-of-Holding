...
export function LibraryScreen({ onNavigate, onToast, pendingSearch, onSelectDoc, selectedId, onSelectCard, selectedCardId, visiblePlanes }) {
  let root;
  ...
  function build() {
    ...
    const ctx = { onNavigate, rebuild, onSelectDoc, selectedId, onSelectCard, selectedCardId, visiblePlanes };
    root = h("div", { class: "content content-narrow" },
      h("div", { class: "page-head" },
        h("div", { class: "t-display" }, "Library"),
        h("div", { class: "sub t-small" }, "Browse documents, search the corpus, and inspect PlaneCards.")),
...
function DocumentsTab({ onNavigate, rebuild, onSelectDoc, selectedId }) {
  const wrap = loading();
  api(`/api/docs?page=${_page}&per_page=${PER}`).then(d => {
...
function SearchTab({ rebuild, onSelectDoc, selectedId }) {
  let inputEl;
  ...
  const searchBar = h("div", { class: "flex gap-2 items-center", style: { marginTop: "16px" } },
    (inputEl = h("input", { type: "text", placeholder: "Search corpus, titles, summaries…", value: _query, "aria-label": "Search query",
      ...
      onKeydown: e => { if (e.key === "Enter") doSearch(e.target.value, rebuild); },
      onInput: e => { _query = e.target.value; } })),
    Button({ variant: "secondary", className: "sm", onClick: () => doSearch(inputEl.value, rebuild), children: ["Search"] }));
  if (_autoRun && _query && !_queryResults) { _autoRun = false; doSearch(_query, rebuild); }
  return h("div", null, searchBar, resultsWrap);
}

function doSearch(q, rebuild) {
  _query = q.trim();
  if (!_query) return;
  _queryResults = null;
  api(`/api/search?q=${encodeURIComponent(_query)}&limit=30`).then(d => { _queryResults = d; rebuild(); });
}
...
function CardsTab({ onSelectCard, selectedCardId, visiblePlanes }) {
  const wrap = loading();
  api("/api/planes/cards?limit=200").then(d => {
    if (d.error) { err(wrap, d.error); return; }
    const cards = d.cards || [];
