 ...
 *  time_norm (Timeline). */
export async function fetchFoldGraph() {
  const [lib, web] = await Promise.all([
    api("/api/fold/library"),
    api("/api/graph/projection?mode=web&max_nodes=300"),
  ]);
  ...
  for (const d of (lib && lib.docs) || []) {
    ...
    byId[d.doc_id] = {
      id: d.doc_id, label: d.title || d.doc_id, kind: "document", evKind: "claim",
  ...
  wnodes.forEach((n, i) => {
    ...
    const nd = byId[id] || (byId[id] = {
      id, label: n.label || n.title || id, kind: "document", evKind: "claim",
      ...
      plane: null, cert: false, bx: null, by: null, time_norm: null,
    });
  ...
  Object.entries(projectGroups).forEach(([project, members]) => {
    ...
    members.forEach(nd => { nd.hidden = true; nd.cluster = cid; });
...
export async function fetchFoldCluster(axis, value) {
  return api(`/api/fold/cluster/${encodeURIComponent(axis)}/${encodeURIComponent(value)}`);
}

/** Full fold packet for one document (inspector detail). */
export async function fetchFoldNode(docId) {
  return api(`/api/fold/node/${encodeURIComponent(docId)}`);
}
