...
domain axis: membership is resolved read-only from registered substrate domains
and existing document/PlaneCard topic tokens. No schema migration or write path.
...
from app.core.domain_linkage import doc_domain_map, normalize_domain, registered_domains
...
def _docs_scope_clause(
    library_id: str | None,
    alias: str = "",
) -> tuple[str, list[str], Any]:
    """Return the user-visible docs predicate for fold browsing."""
    from app.core import promoted_exposure

    exposure_clause = promoted_exposure.exclusion_sql(
        alias,
        show_promoted=promoted_exposure.env_gate_open(),
    )
    library_clause, library_params, library = docs_where_clause(library_id, alias=alias)
    return exposure_clause + library_clause, library_params, library


def _doc_in_scope(doc_id: str, library_id: str | None) -> bool:
    clause, params, _library = _docs_scope_clause(library_id, alias="d")
    row = db.fetchone(
        "SELECT d.doc_id FROM docs d WHERE d.doc_id = ?" + clause,
        (doc_id, *params),
    )
    return bool(row)


def _visible_doc_ids(library_id: str | None) -> set[str]:
    clause, params, _library = _docs_scope_clause(library_id, alias="d")
    return {
        row["doc_id"]
        for row in _safe_fetchall(
            "SELECT d.doc_id FROM docs d WHERE 1=1" + clause,
            tuple(params),
        )
        if row.get("doc_id")
    }
...
def get_current_fold(
    ...
    """
    try:
        visible = _doc_in_scope(doc_id, library_id)
...
def get_fold_library(
    limit: int = Query(500, ge=1, le=2000, description="Max docs to include"),
    library_id: Optional[str] = Query(None, description="Logical library browsing scope"),
):
    ...
    try:
        scope_clause, scope_params, library = _docs_scope_clause(library_id)
    except InvalidLibraryId as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    raw_docs = _safe_fetchall(
        "SELECT doc_id, path, title, authority_state, status, "
...
def _cluster_members_batch(
    value: str,
    library_id: str | None = None,
) -> list[FoldMemberInput]:
    ...
    """
    scope_clause, scope_params, _library = _docs_scope_clause(library_id, alias="d")
    ...
    for row in _safe_fetchall(
        "SELECT d.doc_id FROM docs d WHERE 1=1" + scope_clause,
        tuple(scope_params),
    ):
        doc_id = row.get("doc_id") or ""
...
def _cluster_members_domain(
    value: str,
    library_id: str | None = None,
) -> list[FoldMemberInput]:
    """Domain-axis membership via registered domains and doc/card topic tokens.

    A doc can belong to multiple domain clusters. Docs with no registered domain
    token are passed with axis_value=None so the aggregation engine records that
    they are unplaced on this axis rather than silently hiding the gap.
    """
    wanted = normalize_domain(value)
    visible_ids = _visible_doc_ids(library_id)
    domain_map = doc_domain_map()
    members: list[FoldMemberInput] = []
    for doc_id, domains in domain_map.items():
        ...
        if wanted in domains:
            ...
            members.append(FoldMemberInput(
                node_packet=None,
                axis_value=domains[0] if domains else None,
            ))
    return members
...
def _cluster_members(
    ...
    if axis == "project":
        ...
        for row in _safe_fetchall(
            "SELECT d.doc_id, d.project FROM docs d WHERE 1=1" + scope_clause,
            tuple(scope_params),
        ):
        ...
        return _cluster_members_batch(value, library_id=library_id)
    elif axis == "domain":
        return _cluster_members_domain(value, library_id=library_id)
    return members


def _corpus_clusters(axis: str, library_id: str | None = None) -> list[Any]:
    """Compose one cluster packet per distinct value on `axis`."""
    scope_clause, scope_params, _library = _docs_scope_clause(library_id, alias="d")
    if axis == "project":
        rows = _safe_fetchall(
            "SELECT DISTINCT d.project FROM docs d WHERE d.project IS NOT NULL"
            + scope_clause
            + " ORDER BY d.project",
            tuple(scope_params),
        )
        return [
            aggregate_cluster(
                "project",
                r["project"],
                _cluster_members("project", r["project"], library_id=library_id),
            )
    ...
    if axis == "batch":
        rows = _safe_fetchall(
            """SELECT DISTINCT ic.batch_id FROM intake_capabilities ic
               JOIN docs d ON ic.source_ref = d.path
               WHERE NULLIF(TRIM(ic.batch_id), '') IS NOT NULL"""
            + scope_clause
            + " ORDER BY ic.batch_id",
            tuple(scope_params),
        )
        return [
            aggregate_cluster(
                "batch",
                r["batch_id"],
                _cluster_members_batch(r["batch_id"], library_id=library_id),
            )
    ...
    if axis == "domain":
        return [
            aggregate_cluster(
                "domain",
                domain,
                _cluster_members_domain(domain, library_id=library_id),
            )
...
def get_fold_cluster(
    ...
    domain: membership is resolved read-only from registered substrate domains
    and existing document/PlaneCard topic tokens.
    ...
    cluster_value = normalize_domain(value) if axis == "domain" else value
    try:
        members = _cluster_members(axis, cluster_value, library_id=library_id)
    except InvalidLibraryId as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    packet = aggregate_cluster(axis, cluster_value, members)
    return packet.as_dict()
...
def get_fold_corpus(
    ...
    domain: enumerates registered substrate domains and rolls resolved domain
    clusters into an axis-wide corpus packet.
    ...
    try:
        clusters = _corpus_clusters(axis, library_id=library_id)
    except InvalidLibraryId as exc:
        raise HTTPException(status_code=400, detail=str(exc))
