...
class RunRequest(BaseModel):
    ...
    entity_type:    str = "operator"   # for permission check
    entity_id:      str = "operator"


@router.post("/exec/run", summary="Execute a code block (Python or shell)")
def run_block(req: RunRequest, _operator: str = Depends(require_operator),
    ...
    allowed, basis = al.evaluate_authority(actor_id, "execute_code", "workspace", req.workspace_path or os.environ.get("BOH_LIBRARY", "./library"), operator_authorized=True)
    if not allowed:
        al.ledger_event("execute_code", "document", req.doc_id,
                        actor_id=actor_id, authority_result="denied",
                        authority_basis=basis,
                        after={"language": req.language, "block_id": req.block_id},
                        source_route="/api/exec/run")
    ...
    if result.get("error") and not result.get("run_id"):
        try:
            al.ledger_event("execute_code", "document", req.doc_id,
                            ...
                            authority_basis=result.get("error"),
                            after={"language": req.language, "block_id": req.block_id},
                            source_route="/api/exec/run")
        except Exception:
            pass
    ...
    try:
        al.ledger_event("execute_code", "document", req.doc_id,
                        ...
                        authority_basis=basis,
                        after={"language": req.language, "block_id": req.block_id, "run_id": result.get("run_id"), "status": result.get("status")},
                        source_route="/api/exec/run")
    except Exception:
        pass
    return result
...
def list_runs(doc_id: str, limit: int = Query(20, ge=1)):
