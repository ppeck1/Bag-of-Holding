...
DEMO_CAPABILITY_SURFACES = [
    {
        "surface": "Library",
        "what_to_check": "Demo documents appear with project, lifecycle, status, and authority fields.",
        "route_hint": "/api/docs",
    },
    {
        "surface": "Search",
        "what_to_check": "Queries for viability, contradiction, governance, and Daenary terms return scored results.",
        "route_hint": "/api/search",
    },
    {
        "surface": "Visualization",
        "what_to_check": "Web, Evidence State, Risk Map, and Authority Path modes show different projections.",
        "route_hint": "/api/projection",
    },
    {
        "surface": "Custodian Layer",
        "what_to_check": "Review, conflict, contained, canceled, expired, and canonical examples are present.",
        "route_hint": "/api/governance",
    },
    {
        "surface": "Domains",
        "what_to_check": "PlaneCard backfill can wrap demo documents while preserving source refs.",
        "route_hint": "/api/planes/cards",
    },
    {
        "surface": "Retrieval",
        "what_to_check": "Read-only retrieval can return cited chunks with Planar Gate metadata after indexing.",
        "route_hint": "/api/retrieve",
    },
    {
        "surface": "System Status",
        "what_to_check": "Workspace health, autoindex inventory, activity log, and verification remain inspectable.",
        "route_hint": "/api/status",
    },
]

DEMO_NEXT_STEPS = [
    "Open Library and filter by BOH Consolidated Demo.",
    "Search for viability, contradiction, governance, or Daenary.",
    "Open Visualization and switch through Web, Evidence State, Risk Map, and Authority Path.",
    "Open Custodian Layer to inspect review and authority states.",
    "Open Domains and run PlaneCard backfill if the table is empty.",
    "Open Proposed Changes after an authenticated LLM proposal enqueue or Ollama invocation; demo seed does not fabricate queue items.",
    "Open System Status and run Verify after indexing.",
]
...
def create_markdown_note(req: MarkdownNoteRequest,
    ...
    try:
        al.add_document_attribution(result["doc_id"], "created_by", actor_id,
                                    source="input_markdown", evidence={"path": result["path"]})
        al.ledger_event("create_document", "document", result["doc_id"], actor_id=actor_id,
                        authority_result="allowed", authority_basis="local_input",
                        after={"path": result["path"], "indexed": result["indexed"]},
                        source_route="/api/input/markdown")
    except Exception:
        pass
...
async def upload_files(
    ...
    target_folder: str = Form("scratch"),
    intake_mode:   str = Form("scratch"),
    project:       str = Form(""),
    document_class: str = Form(""),
    status:        str = Form(""),
    canonical_layer: str = Form(""),
    title:         str = Form(""),
    provenance:    str = Form("upload"),
    ...
    Accepted extensions include markdown/text/config/markup, JSON/YAML/CSV, HTML,
    notebooks, and DOCX text extraction candidates.
    ...
    actor_id = al.actor_from_env_or_header(x_boh_actor_id, default="bulk_importer")
        ...
        try:
            ...
            if "reason" in result:
                ...
                saved.append({
                    ...
                    "action":      result.get("action") or ("skipped_unchanged" if result.get("skipped") else "imported"),
                    ...
                    try:
                        ...
                        log_event(
                            ...
                            detail=json.dumps({
                                ...
                                "source": "upload",
                                "skip_reason": result.get("skip_reason", "unchanged"),
                            }),
                        )
                    except Exception:
                        pass
                elif result.get("doc_id"):
                    al.add_document_attribution(
                        result["doc_id"],
                        "imported_by",
                        actor_id,
                        source="bulk_import",
                        evidence={"path": result.get("path"), "filename": upload.filename},
                    )
                    al.ledger_event(
                        "import_document",
                        "document",
                        ...
                        authority_basis="operator_token_fallback" if actor_id == "local_operator" else "importer_workflow",
                        after={"path": result.get("path"), "indexed": result.get("indexed"), "skipped": result.get("skipped", False)},
                        source_route="/api/input/upload",
                    )
        except ValueError as e:
...
def recent_intake(limit: int = Query(20, ge=1)):
...
@router.post("/demo-seed", summary="Load the consolidated BOH demo project")
def load_demo_seed(_operator: str = Depends(require_operator)):
    """Create one consolidated local demo for visualization and governance review."""
    demo_notes = [
    ...
    for project, title, body, topics in demo_notes:
        ...
        try:
            ...
            pass
    daenary = _load_daenary_demo_seed_items()
    all_items = created + daenary.get("items", [])
    fold_demo = _seed_folded_node_demo_facets(all_items)
    return {
        "ok": True,
        "project": "BOH Consolidated Demo",
        "created": len(all_items),
        "items": all_items,
        "sections": {
            "governance_failure_cases": len(created),
            "daenary_epistemic_demo": daenary.get("created", 0),
        },
        "errors": daenary.get("errors", []),
        "includes_failure_cases": True,
        "includes_daenary_epistemic_state": True,
        "capability_surfaces": DEMO_CAPABILITY_SURFACES,
        "next_steps": DEMO_NEXT_STEPS,
        "note": "Demo data is intentionally non-canonical and exists to display governed dimensional storage surfaces.",
        "folded_node_demo": fold_demo,
    }


def _seed_folded_node_demo_facets(items: list[dict]) -> dict:
    """Make one demo node visibly meaningful across folded-node facets."""
    try:
        from app.db import connection as db
        from app.core import plane_card
        from app.core.audit import log_event
        import json
        import time

        anchor = next((i for i in items if i.get("doc_id")), None)
        related = next((i for i in items if i.get("doc_id") and i.get("doc_id") != anchor.get("doc_id")), None) if anchor else None
        if not anchor:
            return {"ok": False, "reason": "no_demo_anchor"}

        doc_id = anchor["doc_id"]
        doc = db.fetchone("SELECT * FROM docs WHERE doc_id = ?", (doc_id,))
        if doc:
            plane_card.wrap_and_persist(dict(doc))

        if related:
            exists = db.fetchone(
                "SELECT id FROM lineage WHERE doc_id = ? AND related_doc_id = ? AND relationship = ?",
                (doc_id, related["doc_id"], "demo_fold_reference"),
            )
            if not exists:
                db.execute(
                    ...
                    (
                        ...
                        "demo_fold_reference",
                        int(time.time()),
                        json.dumps({"source": "demo_seed", "facet": "folded_node"}),
                    ),
                )

            pair_json = json.dumps([doc_id, related["doc_id"]])
            conflict_exists = db.fetchone(
                "SELECT rowid FROM conflicts WHERE conflict_type = ? AND doc_ids = ?",
                ("demo_fold_conflict", pair_json),
            )
            if not conflict_exists:
                db.execute(
                    "INSERT INTO conflicts(conflict_type, doc_ids, term, plane_path, detected_ts) VALUES (?,?,?,?,?)",
                    ("demo_fold_conflict", pair_json, "folded-node-demo", "demo.governed_storage", int(time.time())),
                )

        log_event(
            "demo_folded_node_facets_seeded",
            actor_type="system",
            actor_id="demo_seed",
            doc_id=doc_id,
            detail=json.dumps({"facets": ["source", "lifecycle", "authority", "provenance", "conflicts", "chunks", "plane_card", "planar_gate", "audit"]}),
        )
        return {"ok": True, "anchor_doc_id": doc_id, "related_doc_id": related.get("doc_id") if related else None}
    except Exception as exc:
        return {"ok": False, "reason": str(exc)}


def _load_daenary_demo_seed_items():
    ...
    SEEDS = [
        ...
        ("Canonical Viability Reference", "BOH Consolidated Demo", "canonical", "canonical",
        ...
        ("Ambiguous Evidence Node", "BOH Consolidated Demo", "supporting", "draft",
        ...
        ("Contradicted Claim", "BOH Consolidated Demo", "conflict", "conflict",
        ...
        ("Expired Reference Record", "BOH Consolidated Demo", "supporting", "draft",
        ...
        ("High-Stakes Decision Node", "BOH Consolidated Demo", "review", "review_required",
        ...
        # 6. Low-cost approved
        ("Low-Cost Approved Note", "BOH Consolidated Demo", "supporting", "approved_patch",
        ...
        ("Contextually Ambiguous Note", "BOH Consolidated Demo", "supporting", "draft",
        ...
        ("Likely Incorrect Import", "BOH Consolidated Demo", "quarantine", "quarantine",
        ...
        ("Unreliable Source Intake", "BOH Consolidated Demo", "quarantine", "draft",
        ...
        ("Strong Supporting Record", "BOH Consolidated Demo", "supporting", "draft",
        ...
        ("Pending Correction Note", "BOH Consolidated Demo", "review", "review_required",
        ...
        # 14. Negated evidence
        ("Negated Evidence Record", "BOH Consolidated Demo", "conflict", "conflict",
        ...
        ("Raw Imported Document", "BOH Consolidated Demo", "quarantine", "draft",
        ...
        ("Low-Quality Noise Record", "BOH Consolidated Demo", "quarantine", "scratch",
        ...
        ("Archived Superseded Version", "BOH Consolidated Demo", "archive", "superseded",
        ...
        ("Escalated Review Case", "BOH Consolidated Demo", "review", "review_required",
        ...
        ("Contain Boundary Node", "BOH Consolidated Demo", "supporting", "draft",
        ...
        ("Overconfident Weak Node", "BOH Consolidated Demo", "quarantine", "draft",
        ...
        ("Contested Low-Validity Node", "BOH Consolidated Demo", "conflict", "conflict",
