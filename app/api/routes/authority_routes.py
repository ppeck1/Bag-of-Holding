...
def api_promote(req: PromoteRequest, _operator: str = Depends(require_operator)):
...
def api_list_promotions(target_id: Optional[str] = Query(None), limit: int = Query(100, ge=1)):
...
def api_gated_resolve(item_id: str, req: GatedResolveRequest, _operator: str = Depends(require_operator)):
    actor = ActorProfile(**req.actor.model_dump())
...
def api_gated_resume(req: GatedResumeRequest, _operator: str = Depends(require_operator)):
    actor = ActorProfile(**req.actor.model_dump())
