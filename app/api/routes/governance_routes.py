...
def upsert_policy(req: PolicyRequest, _operator: str = Depends(require_operator),
    ...
    allowed, basis = al.evaluate_authority(actor_id, "mutate_policy", "workspace", req.workspace, operator_authorized=True)
    if not allowed:
        al.ledger_event("mutate_policy", "workspace", req.workspace, actor_id=actor_id,
                        authority_result="denied", authority_basis=basis,
                        after=req.model_dump(), source_route="/api/governance/policy")
        raise HTTPException(status_code=403, detail=f"actor lacks authority: {basis}")
    try:
        result = governance.upsert_policy(
        ...
        al.ledger_event("mutate_policy", "workspace", req.workspace,
                        actor_id=actor_id,
                        authority_result="allowed", authority_basis=basis,
                        after=result, source_route="/api/governance/policy")
        return result
...
def add_system_edge(req: SystemEdgeRequest, _operator: str = Depends(require_operator),
                    x_boh_actor_id: Optional[str] = Header(default=None, alias="X-BOH-Actor-ID")):
    ...
    """
    try:
        result = governance.add_system_edge(
        ...
        al.ledger_event("mutate_system_edge", "system_edge", result.get("id") if isinstance(result, dict) else None,
                        actor_id=al.actor_from_env_or_header(x_boh_actor_id),
                        authority_result="allowed", authority_basis="operator_token_fallback",
                        after=result, source_route="/api/governance/edges")
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def resolve_with_authority(req: GovernanceResolveRequest, _operator: str = Depends(require_operator),
                           x_boh_actor_id: Optional[str] = Header(default=None, alias="X-BOH-Actor-ID")):
    ...
    if result.get("ok"):
        al.ledger_event("resolve_governance_state", "open_item", req.item_id,
                        actor_id=al.actor_from_env_or_header(x_boh_actor_id, default=actor.actor_id),
                        authority_result="allowed", authority_basis="authority_guard",
                        after=result, source_route="/api/governance/resolve")
        return {"status": "resolved", "authority_valid": True, **result}

    if result.get("authorized") is False:
        al.ledger_event("resolve_governance_state", "open_item", req.item_id,
                        actor_id=al.actor_from_env_or_header(x_boh_actor_id, default=actor.actor_id),
                        authority_result="denied", authority_basis="authority_mismatch",
                        after=result, source_route="/api/governance/resolve")
        detail = {
            "status": "rejected",
