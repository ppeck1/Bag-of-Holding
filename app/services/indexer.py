...
class _HTMLToMarkdown(HTMLParser):
    """Small deterministic HTML extractor for indexing, not a lossless converter."""
    def __init__(self):
    ...
    def handle_endtag(self, tag):
        ...
        if self._skip:
            return
        if tag in {"h1", "h2", "h3", "h4", "p", "div", "section", "article"}:
            self.parts.append("\n")
        elif tag == "a":
            ...
            if self._rows:
                ...
                rows = [r + [""] * (max_cols - len(r)) for r in self._rows[:40]]
                self.parts.append("| " + " | ".join(rows[0]) + " |\n")
                self.parts.append("| " + " | ".join(["---"] * max_cols) + " |\n")
                for row in rows[1:]:
    ...
    def handle_data(self, data):
        ...
        elif self._table_depth and self._cell is not None:
...
def _preprocess_for_index(path: Path, text: str) -> str:
    ...
    if suffix in ('.html', '.htm'):
        ...
        conversion_note = (
            "> BOH HTML extraction: deterministic index text, not a lossless conversion.\n"
            "> Extraction mode: html_extraction\n"
            f"> Source file: {path.name}\n"
            f"> Source SHA-256: {source_hash}\n"
            f"> Extracted characters: {len(body)}\n\n"
        )
        return (
    ...
    if suffix == '.ipynb':
        ...
        try:
            body = _extract_ipynb_for_index(_json.loads(text))
            note = "> Notebook code was indexed as inert text; outputs were dropped.\n\n"
        except Exception as exc:
            body = f"Notebook extraction failed: {exc}"
            note = "> Notebook extraction failed; content may be malformed.\n\n"
        return (
            f"---\nboh:\n  id: ipynb-{path.stem[:40]}\n  type: reference\n"
            f"  purpose: {title[:120]}\n  status: draft\n  version: \"0.1.0\"\n"
            f"  updated: \"2026-01-01T00:00:00Z\"\n  rubrix:\n"
            f"    operator_state: observe\n    operator_intent: capture\n---\n\n"
            f"# {title}\n\n{note}{body[:4000]}"
    ...
    if suffix == '.docx':
        title = path.stem.replace('-', ' ').replace('_', ' ').title()
        try:
            body = _extract_docx_for_index(path)
            note = "> DOCX text extraction: layout, formatting, macros, and embedded media are ignored.\n\n"
        except ValueError as exc:
            body = f"DOCX extraction failed: {exc}"
            note = "> DOCX extraction failed; content may be malformed.\n\n"
        return (
            f"---\nboh:\n  id: docx-{path.stem[:40]}\n  type: reference\n"
            f"  purpose: {title[:120]}\n  status: draft\n  version: \"0.1.0\"\n"
            f"  updated: \"2026-01-01T00:00:00Z\"\n  rubrix:\n"
            f"    operator_state: observe\n    operator_intent: capture\n---\n\n"
            f"# {title}\n\n{note}{body[:4000]}"
    ...
    if suffix in _TEXT_INDEX_EXTENSIONS:
...
_TEXT_INDEX_EXTENSIONS = {
    ".txt", ".rst", ".mdx", ".tex", ".bib", ".log",
    ".toml", ".ini", ".cfg", ".conf", ".properties", ".env.example", ".xml",
}


def _index_extension(path: Path) -> str:
    suffixes = [s.lower() for s in path.suffixes]
    supported = _TEXT_INDEX_EXTENSIONS | {
        ".md", ".markdown", ".html", ".htm", ".json", ".jsonl", ".yaml", ".yml",
        ".csv", ".tsv", ".ipynb", ".docx",
    }
...
def _extract_ipynb_for_index(data: object) -> str:
    if not isinstance(data, dict) or not isinstance(data.get("cells"), list):
        raise ValueError("no cells list")
    chunks: list[str] = []
    for cell in data["cells"]:
        ...
        if cell.get("cell_type") == "code":
            ...
            chunks.append(text)
    if not chunks:
        raise ValueError("no extractable cell source")
    return "\n\n".join(chunks)


def _extract_docx_for_index(path: Path) -> str:
    try:
    ...
    for para in root.iter(f"{ns}p"):
        text = "".join(node.text or "" for node in para.iter(f"{ns}t")).strip()
        if text:
...
def index_file(path: Path, library_root: Path) -> dict:
    ...
    try:
        ...
        # Retrieval v1: stable chunk index for read-only LLM context packs.
        try:
            from app.core.retrieval import replace_doc_chunks
            replace_doc_chunks(
                conn,
                {
                    "doc_id": doc_id,
                    ...
                    "operator_state": rubrix.get("operator_state"),
                    "authority_state": contract.get("authority_state"),
                    "status": contract.get("status"),
                    "canonical_layer": contract.get("canonical_layer"),
                },
                body,
                frontmatter_text=json.dumps(raw_header or {}, indent=2),
            )
        except Exception:
            # Chunks are rebuildable; document indexing remains authoritative.
            pass
    ...
    try:
        ...
        from app.core.promoted_exposure import PROMOTED_CORPUS_CLASS
        full_indexed_doc = db.fetchone("SELECT * FROM docs WHERE doc_id = ?", (doc_id,))
        # WO-2: promoted intake derivatives get NO PlaneCard — a card would re-expose the doc
        # through card surfaces and retrieval card packs while the exposure gates are closed,
        # and demotion would orphan it (defect found by the first-promotion gate, 2026-06-11).
        if full_indexed_doc and full_indexed_doc.get("corpus_class") != PROMOTED_CORPUS_CLASS:
...
def crawl_library(library_root: str) -> dict:
    ...
    SUPPORTED_GLOBS = [
        "*.md", "*.markdown", "*.mdx", "*.html", "*.htm", "*.json", "*.jsonl",
        "*.txt", "*.rst", "*.csv", "*.tsv", "*.yaml", "*.yml", "*.toml", "*.ini",
        "*.cfg", "*.conf", "*.properties", "*.env.example", "*.xml", "*.tex",
        "*.bib", "*.log", "*.ipynb", "*.docx",
    ]
    seen = set()
    all_paths = []
