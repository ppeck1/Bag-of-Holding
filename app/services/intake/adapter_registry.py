...
def _extension_candidates(path: str) -> list[str]:
    """Return extension candidates, longest compound suffix first."""
    suffixes = Path(path).suffixes
    out: list[str] = []
    for i in range(len(suffixes)):
        out.append("".join(suffixes[i:]).lower())
    if not out:
...
class AdapterRegistry:
    ...
    def resolve(self, path: str, media_type: str | None = None) -> tuple[AdapterMetadata, str | None, str | None]:
        ...
        meta = None
        for candidate_ext in _extension_candidates(path):
            meta = self.match_extension(candidate_ext)
            if meta is not None:
                ext = candidate_ext
                break
