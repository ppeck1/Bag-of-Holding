...
const BOH_VERSION = 'phase28.4-acceptance-repro-ui-hardening';
console.info(`%c📦 Bag of Holding ${BOH_VERSION}`, 'color:#10b981;font-weight:bold;font-size:14px');
console.info(`BOH UI loaded: ${BOH_VERSION}`);
...
const OPERATOR_TOKEN_STORAGE_KEY = 'boh_operator_token';
const ACTOR_ID_STORAGE_KEY = 'boh_actor_id';
const OPERATOR_HEADER_NAME = 'X-BOH-Operator-Token';
const ACTOR_HEADER_NAME = 'X-BOH-Actor-ID';
...
/**
 * Full-width table status row (loading / empty / error message).
 * Mirrors the repeated `<tr><td colspan=N class="text-faint" …>…</td></tr>` idiom.
 *   center=true  → `padding:16px;text-align:center` (ledger / intake style)
 *   center=false → `padding:12px` (compact dashboard style)
 */
function tableStatusRow(colspan, content, center = true) {
  const style = center ? 'padding:16px;text-align:center' : 'padding:12px';
  return `<tr><td colspan="${colspan}" class="text-faint" style="${style}">${content}</td></tr>`;
}

/**
 * Render data rows into a <tbody> by element id, with standard empty-state
 * handling. `rowFn` maps one item to a `<tr>…</tr>` string (identical to the
 * inline `.map()` callbacks it replaces), so rendered output is unchanged.
 * No-op when the target element is absent (preserves prior `if (el(id))` guards).
 */
function renderTable(targetId, rows, rowFn, { colspan = 1, empty = 'No items.', center = true } = {}) {
  const node = el(targetId);
  if (!node) return;
  node.innerHTML = (rows && rows.length)
    ? rows.map(rowFn).join('')
    : tableStatusRow(colspan, empty, center);
}
...
async function api(path, opts = {}) {
  try {
    const r = await fetch(path, withOperatorHeaders(path, opts));
    const ct = r.headers.get('content-type') || '';
    const payload = ct.includes('application/json') ? await r.json() : await r.text();
    if (!r.ok) {
      const detail = typeof payload === 'object' ? (payload.detail || payload.error || JSON.stringify(payload)) : payload;
      return { error: `HTTP ${r.status}: ${detail || r.statusText}`, status: r.status, detail };
    }
    return payload;
  } catch (e) {
...
function operatorHeaders() {
  ...
  const headers = {};
  if (token) headers[OPERATOR_HEADER_NAME] = token;
  const actor = getCurrentActorId();
  if (actor) headers[ACTOR_HEADER_NAME] = actor;
  return headers;
}

function isProtectedApiCall(path, opts = {}) {
  ...
  return [
    ...
    '/api/docs/',
    '/api/workflow/',
    '/api/corpus/reclassify',
    '/api/corpus/migration-report',
    '/api/duplicates/decision',
    '/api/review-queue/',
    '/api/intake/run',
  ].some(prefix => p.startsWith(prefix));
}

function withOperatorHeaders(path, opts = {}) {
  if (!isProtectedApiCall(path, opts)) return opts;
  const headers = new Headers(opts.headers || {});
  const token = getOperatorToken();
  if (token && !headers.has(OPERATOR_HEADER_NAME)) headers.set(OPERATOR_HEADER_NAME, token);
  const actor = getCurrentActorId();
  if (actor && !headers.has(ACTOR_HEADER_NAME)) headers.set(ACTOR_HEADER_NAME, actor);
  return { ...opts, headers };
}

function protectedRequestPreview(path, opts = {}) {
  const prepared = withOperatorHeaders(path, opts);
  const headers = new Headers(prepared.headers || {});
  return {
    path,
    method: String(prepared.method || 'GET').toUpperCase(),
    has_operator_token: headers.has(OPERATOR_HEADER_NAME),
    has_actor_id: headers.has(ACTOR_HEADER_NAME),
    content_type: headers.get('Content-Type') || headers.get('content-type') || '',
  };
}

function saveOperatorToken(inputId = 'operator-token-input') {
  const input = el(inputId);
  const token = input?.value?.trim() || '';
  const statusText = token ? 'Operator token saved for this tab.' : 'Enter a token first.';
  if (token) {
    try { sessionStorage.setItem(OPERATOR_TOKEN_STORAGE_KEY, token); } catch (_) {}
    if (input) input.value = '';
  }
  updateOperatorTokenStatus(statusText);
}

function clearOperatorToken() {
  try { sessionStorage.removeItem(OPERATOR_TOKEN_STORAGE_KEY); } catch (_) {}
  const a = el('operator-token-input');
  const b = el('operator-token-input-status');
  if (a) a.value = '';
  if (b) b.value = '';
  updateOperatorTokenStatus('Operator token cleared.');
}

function updateOperatorTokenStatus(message) {
  const saved = !!getOperatorToken();
  const text = message || (saved ? 'Operator token saved for this tab.' : 'Operator token not saved.');
  for (const id of ['operator-token-status', 'operator-token-status-status']) {
    const node = el(id);
    if (node) {
      node.textContent = text;
      node.style.color = saved ? 'var(--green-bright)' : 'var(--text-faint)';
    }
  }
}
...
function getCurrentActorId() {
  try {
    return sessionStorage.getItem(ACTOR_ID_STORAGE_KEY) || 'local_operator';
  } catch (_) {
    return 'local_operator';
  }
}

function saveCurrentActor(inputId = 'current-actor-input') {
  const input = el(inputId);
  const actor = (input?.value || '').trim() || 'local_operator';
  try { sessionStorage.setItem(ACTOR_ID_STORAGE_KEY, actor); } catch (_) {}
  updateActorStatus();
}

function updateActorStatus() {
  const actor = getCurrentActorId();
  for (const id of ['current-actor-status', 'current-actor-status-status']) {
    const node = el(id);
    if (node) node.textContent = `Current actor: ${actor}`;
  }
  for (const id of ['current-actor-input', 'current-actor-input-status']) {
    const input = el(id);
    if (input && !input.value) input.placeholder = actor;
  }
}

async function classifyProtectedError(response, actionLabel = 'Protected action') {
  const status = response?.status;
  const detail = String(response?.detail || response?.error || '').toLowerCase();
  const op = await api('/api/operator/status');
  if (op?.dev_open) return `${actionLabel} failed: ${formatApiError(response)}`;
  if (op?.configured === false) return 'Operator authorization is not configured. Set BOH_OPERATOR_TOKEN before launch.';
  if (!getOperatorToken() || (status === 401 && detail.includes('missing'))) return 'Operator token required. Enter and save the local operator token.';
  if (status === 401 || detail.includes('invalid operator') || detail.includes('token rejected')) return 'Operator token rejected.';
  if (!getCurrentActorId()) return 'Actor ID missing. Defaulting to local_operator.';
  if (status === 403 && detail.includes('lacks authority')) return 'Current actor lacks reset_workspace authority.';
  return `${actionLabel} failed: ${formatApiError(response)}`;
}

function renderResetReport(report) {
  if (!report || report.error) return escHtml(formatApiError(report));
  const rows = [
    ['Files removed', (report.files_removed || []).length],
    ['Files quarantined', (report.files_quarantined || []).length],
    ['Artifacts removed', (report.artifacts_removed || []).length],
    ['Stale refs removed', (report.stale_refs_removed || []).length],
    ['Reindex status', report.reindex_status || 'skipped'],
    ['Warnings', (report.warnings || []).length],
  ];
  const dbRows = report.db_rows_removed || {};
  const dbSummary = Object.keys(dbRows).length
    ? Object.entries(dbRows).slice(0, 8).map(([k, v]) => `${escHtml(k)}=${escHtml(String(v))}`).join(', ')
    : 'none';
  return `
    <div style="color:var(--green-bright);font-weight:600">Clean Test Workspace completed.</div>
    <div class="kv-grid font-sm" style="margin-top:8px">
      ${rows.map(([k, v]) => `<div class="kv-key">${escHtml(k)}</div><div class="kv-val">${escHtml(String(v))}</div>`).join('')}
      <div class="kv-key">DB rows removed</div><div class="kv-val">${dbSummary}</div>
    </div>
    ${(report.warnings || []).length ? `<div class="text-amber font-sm" style="margin-top:6px">${(report.warnings || []).map(w => escHtml(w)).join('<br>')}</div>` : ''}
  `;
}

async function bohProtectedFetch(path, opts = {}) {
  return fetch(path, withOperatorHeaders(path, { ...opts, method: opts.method || 'POST' }));
}
...
function applyCopyMap() {
  ...
  if (copy.appName || copy.appSubtitle) {
    ...
    if (wordmark) wordmark.innerHTML = `
      <img class="wordmark-icon" src="assets/favicons/favicon-32x32.png" alt="" aria-hidden="true">
      <span class="wordmark-text">${escHtml(copy.appName || 'Bag of Holding')}</span>
      <span class="wordmark-subtitle">${escHtml(copy.appSubtitle || 'Governed dimensional storage for durable reasoning.')}</span>`;
  }
...
async function loadDemoProject() {
  ...
  const sectionText = r.ok && Object.keys(sections).length
    ? ` (${Object.entries(sections).map(([k,v]) => `${escHtml(k.replace(/_/g, ' '))}: ${escHtml(String(v))}`).join(', ')})`
    : '';
  const surfaces = Array.isArray(r.capability_surfaces) ? r.capability_surfaces : [];
  const nextSteps = Array.isArray(r.next_steps) ? r.next_steps : [];
  const surfaceText = surfaces.length
    ? `<div class="text-faint" style="margin-top:4px">Capability surfaces: ${surfaces.map(s => escHtml(s.surface || '')).filter(Boolean).join(' · ')}</div>`
    : '';
  const stepText = nextSteps.length
    ? `<div class="text-faint" style="margin-top:4px">Next: ${nextSteps.slice(0, 3).map(escHtml).join(' · ')}</div>`
    : '';
  if (status) status.innerHTML = r.ok
    ? `<span class="text-green">✓ Demo loaded: ${escHtml(String(r.created || 0))} items${sectionText}</span>${surfaceText}${stepText}`
    : `<span class="text-red">${escHtml(r.error || r.detail || 'Demo load failed')}</span>`;
...
function nav(panelId) {
  ...
  if (navItem) {
    ...
    // Auto-open Advanced Tools if the active item lives inside it
    const advSection = el('nav-adv');
    if (advSection && advSection.contains(navItem) && advSection.style.display === 'none') {
      toggleAdvNav();
    }
  }
  ...
  if (panelId === 'fold-view') safeLoad(panelId, () => loadFoldLibraryView());
  if (panelId === 'trace-ledger') safeLoad(panelId, loadTraceLedger);
  if (panelId === 'review-queue') safeLoad(panelId, loadReviewQueue);
  if (panelId === 'residence-map') safeLoad(panelId, loadResidenceMap);
  if (panelId === 'context-pack-builder') safeLoad(panelId, loadContextPackBuilder);
  if (panelId === 'inbox') safeLoad(panelId, loadInbox);
  if (panelId === 'quarantine') safeLoad(panelId, loadQuarantine);
  if (panelId === 'authority-ledger') safeLoad(panelId, loadAuthorityLedger);
}
...
// Toggle the Advanced Tools collapsible section in the sidebar
function toggleAdvNav() {
  const adv  = el('nav-adv');
  const chev = el('nav-adv-chevron');
  const tog  = el('nav-adv-toggle');
  if (!adv) return;
...
async function loadIntegrityDashboard() {
  ...
  renderTable('integrity-risk-body', risk, r => {
  ...
  renderTable('integrity-open-items-body', openItems, i => `<tr>
    ...
    </tr>`, { colspan: 4, empty: 'No open registry items.', center: false });
}
...
async function doIndex() {
  const root = './library';
  const btn = el('index-btn') || document.querySelector('button[onclick="doIndex()"]') || { disabled:false, innerHTML:'', textContent:'' };
  if (btn) btn.disabled = true;
  btn.innerHTML = '<span class="spinner"></span> Indexing…';
  el('index-result').innerHTML = '';

  const data = await api('/api/autoindex/run', {
    method: 'POST',
    headers: {'Content-Type':'application/json'},
    body: JSON.stringify({ changed_only: true }),
  });

  if (btn) btn.disabled = false;
  if (btn) btn.textContent = 'Re-index library';
  ...
  let html = `
    <div class="kv-grid font-sm" style="margin-bottom:12px;padding:12px;background:var(--bg-base);border-radius:4px;border:1px solid var(--border-dim)">
      <div class="kv-key">Scanned</div><div class="kv-val text-bright">${data.scanned ?? data.total_files ?? 0}</div>
      <div class="kv-key">Indexed</div><div class="kv-val text-green">${data.indexed}</div>
      <div class="kv-key">Skipped unchanged</div><div class="kv-val">${data.skipped ?? 0}</div>
      <div class="kv-key">Failed</div><div class="kv-val ${(data.failed||0)>0?'text-red':''}">${data.failed ?? 0}</div>
    </div>`;
...
async function openDocInReader(docId) {
  ...
  renderFoldSnapshot(docId).catch(() => {
    const snap = el('reader-fold-snapshot');
    if (snap) snap.classList.add('hidden');
  });
  renderFoldedNodePacket(docId).catch(() => {
    const fold = el('reader-fold');
...
async function renderFoldedNodePacket(docId) {
  const fold = el('reader-fold');
  if (!fold) return;
  fold.classList.remove('hidden');
  fold.innerHTML = '<div class="text-faint font-sm"><span class="spinner"></span> Loading folded node…</div>';
  const packet = await api(`/api/docs/${encodeURIComponent(docId)}/fold`);
  if (!packet || packet.error || packet.detail) {
    fold.classList.add('hidden');
    return;
  }
  const s = packet.summary || {};
  const f = packet.facets || {};
  const gate = f.planar_gate?.gate_result || {};
  const card = f.plane_card || {};
  const chunks = f.chunks || {};
  const conflicts = f.conflicts || {};
  const audit = f.audit || {};
  const latestAudit = (audit.audit_log || [])[0] || (audit.storage_events || [])[0] || {};
  const firstChunk = (chunks.items || [])[0] || {};
  const lineages = f.provenance?.lineage || [];
  const source = f.source || {};
  const auth = f.authority || {};
  const lifecycle = f.lifecycle || {};

  const facet = (name, lines) => `<div class="fold-facet"><h4>${escHtml(name)}</h4>${lines.map(x => `<div>${escHtml(String(x ?? '—'))}</div>`).join('')}</div>`;
  fold.innerHTML = `
    <div class="fold-summary">
      <span class="fold-title">Folded Node</span>
      <span class="fold-chip">risk: ${escHtml(s.highest_risk_state || 'unknown')}</span>
      <span class="fold-chip">authority: ${escHtml(s.authority_state || '—')}</span>
      <span class="fold-chip">chunks: ${escHtml(String(s.chunk_count || 0))}</span>
      <span class="fold-chip">gate: ${escHtml(gate.posture || '—')}</span>
    </div>
    <div class="fold-facets">
      ${facet('Source', [source.path || packet.doc_id, source.type || 'document', source.source_hash ? `hash ${String(source.source_hash).slice(0, 16)}` : 'hash unavailable'])}
      ${facet('Lifecycle', [lifecycle.operator_state || 'unknown', lifecycle.operator_intent || 'intent unknown', `${(lifecycle.history || []).length} history events`])}
      ${facet('Authority', [auth.authority_state || 'non_authoritative', auth.review_state || 'review unknown', auth.canonical_layer || 'layer unknown'])}
      ${facet('Provenance', [`${lineages.length} lineage links`, `planes: ${(f.provenance?.plane_scope || []).join(', ') || 'none'}`])}
      ${facet('Conflicts', [`${conflicts.count || 0} open/known`, (conflicts.items || [])[0]?.conflict_type || 'none visible'])}
      ${facet('Chunks', [`${chunks.count || 0} chunks`, firstChunk.heading_path || firstChunk.chunk_type || 'no chunk preview'])}
      ${facet('PlaneCard', [card.present ? card.card_id : 'not generated', card.plane || 'plane unknown', card.card_type || 'type unknown'])}
      ${facet('Planar Gate', [gate.posture || 'not evaluated', (gate.warning_reasons || []).join(', ') || 'no warnings', gate.required_route || 'no route required'])}
      ${facet('Audit', [latestAudit.event_type || 'no recent event', latestAudit.actor_id || latestAudit.subject_id || 'actor unknown'])}
    </div>`;
}

// ── Current Fold View (Phases 0-5) ─────────────────────────────────────────

async function renderFoldSnapshot(docId) {
  const snap = el('reader-fold-snapshot');
  if (!snap) return;
  snap.classList.remove('hidden');
  snap.innerHTML = '<div class="text-faint font-sm"><span class="spinner"></span> Loading fold state…</div>';
  let packet;
  try {
    packet = await api(`/api/fold/node/${encodeURIComponent(docId)}`);
  } catch (_) {
  ...
  const sym = packet.symbolic_state || {};
  const sc = packet.scalar_state || {};
  const unknowns = packet.unknowns || [];
  const label = sym.currentness_label || 'unknown';
  const labelColors = {
    current: '#2ecc71', draft_current: '#27ae60', current_but_contested: '#f39c12',
    stale: '#e67e22', conflicted: '#e74c3c', superseded: '#9b59b6',
    held: '#3498db', quarantined: '#c0392b', unknown: '#7f8c8d',
  };
  const col = labelColors[label] || 'var(--text-muted)';
  const scalarBar = (name, val) => {
    const pct = Math.round((val || 0) * 100);
    return `<div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
      <span style="width:120px;font-size:10px;color:var(--text-faint)">${escHtml(name)}</span>
      <div style="flex:1;height:4px;background:var(--border-dim);border-radius:2px">
        <div style="width:${pct}%;height:4px;background:var(--blue);border-radius:2px"></div>
      </div>
      <span style="font-size:10px;width:30px;text-align:right">${pct}%</span>
    </div>`;
  };
  const unknownBadges = unknowns.slice(0, 3).map(u =>
    `<span style="font-size:10px;padding:1px 5px;border-radius:3px;background:var(--border-soft);color:var(--text-muted)" title="${escHtml(u.meaning)}">${escHtml(u.field)}</span>`
  ).join(' ');

  snap.innerHTML = `
    <div style="font-size:10px;color:var(--text-faint);letter-spacing:.1em;text-transform:uppercase;margin-bottom:6px">Fold Snapshot</div>
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
      <span style="font-size:12px;font-weight:600;padding:2px 8px;border-radius:4px;background:${col};color:#fff">${escHtml(label)}</span>
      <span style="font-size:10px;color:var(--text-faint)">${escHtml(sym.authority_label || '')} &middot; ${escHtml(sym.freshness_label || '')} &middot; ${escHtml(sym.conflict_label || '')}</span>
    </div>
    <div style="font-size:10px;color:var(--text-faint);margin-bottom:4px">Dimensional pressures (not truth scores)</div>
    ${scalarBar('Authority pressure', sc.authority_score)}
    ${scalarBar('Freshness pressure', sc.freshness_score)}
    ${scalarBar('Conflict pressure', sc.conflict_pressure)}
    ${scalarBar('Canon readiness', sc.canon_readiness)}
    ${unknowns.length ? `<div style="margin-top:6px;font-size:10px;color:var(--text-faint)">Unknowns: ${unknownBadges}${unknowns.length > 3 ? ` +${unknowns.length - 3} more` : ''}</div>` : ''}
    <div style="margin-top:4px;font-size:10px;color:var(--text-faint)">scope: node / ${escHtml(docId)} &middot; ${escHtml(packet.cache_status || 'live')}</div>`;
}

// ── Fold View: Scatter Canvas ──────────────────────────────────

const FOLD_LABEL_COLORS = {
  current:              '#2ecc71',
  draft_current:        '#27ae60',
  current_but_contested:'#f39c12',
  stale:                '#e67e22',
  conflicted:           '#e74c3c',
  superseded:           '#9b59b6',
  held:                 '#3498db',
...
let _foldLibData      = null;
let _foldSelected     = null;
let _foldHover        = null;
let _foldHiddenLabels = new Set();
let _foldResizeObs    = null;

async function loadFoldLibraryView(force = false) {
  if (_foldLibData && !force) { _foldDrawLibrary(); return; }
  const overlay = el('fold-view-overlay');
  const msg     = el('fold-view-overlay-msg');
  if (overlay) overlay.style.display = '';
  if (msg)     msg.textContent = 'Loading fold state…';
  let data;
  try {
    data = await api('/api/fold/library');
  } catch (_) {
    if (msg) msg.textContent = 'Failed to load — is the server running?';
    return;
  }
  if (!data || data.error) {
    const errText = data?.error || 'unknown error';
    if (msg) msg.textContent = errText.includes('404')
      ? 'Fold library API not found. Restart the server to pick up new routes.'
      : `API error: ${errText}`;
    return;
  }
  if (!Array.isArray(data.docs)) {
    if (msg) msg.textContent = 'Unexpected API response format.';
    return;
  }
  _foldLibData = data;
  _foldSelected = null;
  _foldHover = null;
  _foldHiddenLabels.clear();
  if (overlay) overlay.style.display = 'none';
  const countEl = el('fold-view-count');
  if (countEl) countEl.textContent = `${data.total} docs`;
  _foldBuildLegend();
  _foldInitCanvas();
  _foldDrawLibrary();
}

function _foldBuildLegend() {
  const legendEl = el('fold-view-legend');
  if (!legendEl || !_foldLibData) return;
  const counts = _foldLibData.label_counts || {};
  legendEl.innerHTML = Object.keys(FOLD_LABEL_COLORS).map(label => {
    const n = counts[label] || 0;
    if (!n) return '';
    const col = FOLD_LABEL_COLORS[label];
    const dim = _foldHiddenLabels.has(label) ? 'opacity:.35;' : '';
    return `<button onclick="toggleFoldLabel('${label}')" title="${label}: ${n} docs"
      style="border:none;cursor:pointer;border-radius:3px;padding:2px 7px;font-size:10px;
             background:${col};color:#fff;${dim}letter-spacing:.03em">${label.replace(/_/g,' ')} <span style="opacity:.75">${n}</span></button>`;
  }).join('');
}

function toggleFoldLabel(label) {
  if (_foldHiddenLabels.has(label)) _foldHiddenLabels.delete(label);
  else _foldHiddenLabels.add(label);
  _foldBuildLegend();
  _foldDrawLibrary();
}

function _foldInitCanvas() {
  const wrap   = el('fold-view-canvas-wrap');
  const canvas = el('fold-view-canvas');
  if (!wrap || !canvas) return;
  _foldResizeCanvas();
  if (_foldResizeObs) _foldResizeObs.disconnect();
  _foldResizeObs = new ResizeObserver(() => { _foldResizeCanvas(); _foldDrawLibrary(); });
  _foldResizeObs.observe(wrap);
  canvas.onmousemove = (e) => {
    const rect = canvas.getBoundingClientRect();
    const hit  = _foldHitTest(e.clientX - rect.left, e.clientY - rect.top, canvas.width, canvas.height, 48);
    if (hit !== _foldHover) {
      _foldHover = hit;
  ...
  canvas.onmouseleave = () => {
    if (_foldHover) { _foldHover = null; _foldDrawLibrary(); }
    canvas.style.cursor = 'default';
  };
  canvas.onclick = (e) => {
    const rect = canvas.getBoundingClientRect();
    const hit  = _foldHitTest(e.clientX - rect.left, e.clientY - rect.top, canvas.width, canvas.height, 48);
    if (hit) _foldSelectDoc(hit);
  };
}

function _foldResizeCanvas() {
  const wrap = el('fold-view-canvas-wrap');
  const canvas = el('fold-view-canvas');
  if (!wrap || !canvas) return;
...
function _foldProject(x, y, W, H, pad) {
  return { px: pad + x * (W - 2 * pad), py: H - pad - y * (H - 2 * pad) };
}

function _foldDrawLibrary() {
  const canvas = el('fold-view-canvas');
  if (!canvas || !_foldLibData) return;
  const dpr = window.devicePixelRatio || 1;
  ...
  for (let i = 0; i <= 4; i++) {
    const { px } = _foldProject(i / 4, 0, W, H, pad);
    ctx.fillText((i * 25) + '%', px, H - pad + 14);
  }
  ctx.textAlign = 'right';
  for (let i = 0; i <= 4; i++) {
    const { py } = _foldProject(0, i / 4, W, H, pad);
    ctx.fillText((i * 25) + '%', pad - 6, py + 3);
  }
  ...
  ctx.textAlign = 'center';
  ctx.fillText('Authority pressure →', W / 2, H - 4);
  ctx.save();
  ctx.translate(11, H / 2);
  ctx.rotate(-Math.PI / 2);
  ctx.fillText('Freshness ↑', 0, 0);
  ctx.restore();

  // Nodes — base pass (skip selected/hovered, draw on top later)
  const docs = _foldLibData.docs || [];
  for (const doc of docs) {
    if (_foldHiddenLabels.has(doc.currentness_label)) continue;
    if (doc === _foldSelected || doc === _foldHover) continue;
    _foldDrawNode(ctx, doc, W, H, pad, false, false);
  }
  if (_foldSelected && !_foldHiddenLabels.has(_foldSelected.currentness_label)) {
    _foldDrawNode(ctx, _foldSelected, W, H, pad, false, true);
  }
  if (_foldHover && _foldHover !== _foldSelected && !_foldHiddenLabels.has(_foldHover.currentness_label)) {
    _foldDrawNode(ctx, _foldHover, W, H, pad, true, false);
  }

  // Tooltip
  if (_foldHover) {
    const { px, py } = _foldProject(_foldHover.authority_score, _foldHover.freshness_score, W, H, pad);
    const title = (_foldHover.title || _foldHover.doc_id);
    _foldDrawTooltip(ctx, px, py, [
      title.length > 38 ? title.slice(0, 36) + '…' : title,
      `${(_foldHover.currentness_label || 'unknown').replace(/_/g,' ')} · auth ${((_foldHover.authority_score || 0) * 100).toFixed(0)}% · fresh ${((_foldHover.freshness_score || 0) * 100).toFixed(0)}%`,
      `canon readiness: ${((_foldHover.canon_readiness || 0) * 100).toFixed(0)}%`,
    ], W, H);
  }
}

function _foldDrawNode(ctx, doc, W, H, pad, hover, selected) {
  const { px, py } = _foldProject(doc.authority_score || 0, doc.freshness_score || 0, W, H, pad);
  const col  = FOLD_LABEL_COLORS[doc.currentness_label] || '#7f8c8d';
  const baseR = 4 + (doc.canon_readiness || 0) * 8;
  const r     = hover ? baseR + 2 : baseR;
  ctx.beginPath();
...
function _foldDrawTooltip(ctx, px, py, lines, W, H) {
  const pad2 = 8;
  const lineH = 14;
  ctx.font = '11px sans-serif';
  const maxW = Math.max(...lines.map(l => ctx.measureText(l).width));
  const tw = maxW + pad2 * 2;
  const th = lines.length * lineH + pad2 * 1.5;
  let tx = px + 14;
  let ty = py - th / 2;
  if (tx + tw > W - 4) tx = px - tw - 14;
  if (ty < 4) ty = 4;
  if (ty + th > H - 4) ty = H - th - 4;
  _foldRoundRect(ctx, tx, ty, tw, th, 4);
  ctx.fillStyle = 'rgba(18,18,22,0.93)';
  ctx.fill();
  ...
  lines.forEach((line, i) => {
    ctx.fillStyle = i === 0 ? '#fff' : 'rgba(200,200,200,0.68)';
    ctx.fillText(line, tx + pad2, ty + pad2 + (i + 0.78) * lineH);
  });
}

function _foldRoundRect(ctx, x, y, w, h, r) {
  ctx.beginPath();
  ctx.moveTo(x + r, y);
  ctx.lineTo(x + w - r, y);
...
function _foldHitTest(mx, my, W, H, pad) {
  if (!_foldLibData) return null;
  const dpr = window.devicePixelRatio || 1;
  const cW = W / dpr;
  const cH = H / dpr;
  let best = null;
  let bestD = Infinity;
  for (const doc of _foldLibData.docs) {
    if (_foldHiddenLabels.has(doc.currentness_label)) continue;
    const { px, py } = _foldProject(doc.authority_score || 0, doc.freshness_score || 0, cW, cH, pad);
    const hitR = 4 + (doc.canon_readiness || 0) * 8 + 4;
    const d = Math.hypot(mx - px, my - py);
    if (d < hitR && d < bestD) { best = doc; bestD = d; }
  }
  return best;
}

async function _foldSelectDoc(doc) {
  _foldSelected = doc;
  _foldDrawLibrary();
  const detail = el('fold-view-detail');
  if (!detail) return;
  const col = FOLD_LABEL_COLORS[doc.currentness_label] || '#7f8c8d';
  detail.innerHTML = `
    <div style="margin-bottom:10px">
      <div style="font-size:14px;font-weight:600;color:var(--text);margin-bottom:2px">${escHtml(doc.title || doc.doc_id)}</div>
      <div style="font-size:10px;color:var(--text-faint);margin-bottom:6px">${escHtml(doc.doc_id)}</div>
      <span style="font-size:11px;padding:2px 8px;border-radius:3px;background:${col};color:#fff">${escHtml((doc.currentness_label || 'unknown').replace(/_/g,' '))}</span>
    </div>
    <div style="margin-bottom:10px">
      <div style="font-size:10px;color:var(--text-faint);margin-bottom:5px">Dimensional pressures <span style="font-style:italic">(not truth scores)</span></div>
      ${_foldScalarBar('Authority pressure', doc.authority_score)}
      ${_foldScalarBar('Freshness pressure', doc.freshness_score)}
      ${_foldScalarBar('Conflict pressure', doc.conflict_pressure)}
      ${_foldScalarBar('Canon readiness', doc.canon_readiness)}
    </div>
    <div style="color:var(--text-faint);font-size:11px"><span class="spinner"></span> Loading full resolver output…</div>`;

  let packet;
  try {
    packet = await api(`/api/fold/node/${encodeURIComponent(doc.doc_id)}`);
  } catch (_) {
    const loading = detail.querySelector('.spinner')?.parentElement;
    if (loading) loading.textContent = 'Full resolver output unavailable.';
    return;
  }
  _foldRenderFullDetail(detail, doc, packet);
}

function _foldScalarBar(name, val) {
  const pct = Math.round((val || 0) * 100);
  return `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
    <span style="width:130px;font-size:10px;color:var(--text-faint);flex-shrink:0">${escHtml(name)}</span>
    <div style="flex:1;height:4px;background:var(--border-dim);border-radius:2px">
      <div style="width:${pct}%;height:4px;background:var(--blue);border-radius:2px"></div>
    </div>
    <span style="font-size:10px;width:28px;text-align:right;flex-shrink:0">${pct}%</span>
  </div>`;
}

function _foldRenderFullDetail(container, doc, packet) {
  if (!packet || packet.detail) {
    const spin = container.querySelector('.spinner');
    if (spin && spin.parentElement) spin.parentElement.textContent = packet?.detail || 'Not found.';
    return;
  }
  const sym  = packet.symbolic_state || {};
  const sc   = packet.scalar_state   || {};
  const trace = packet.resolver_trace_summary || [];
  const unknowns = packet.unknowns || [];
  const col  = FOLD_LABEL_COLORS[sym.currentness_label] || '#7f8c8d';

  const traceRows = trace.map(e =>
    `<tr>
      <td style="font-size:10px;color:var(--text-faint);padding:2px 8px 2px 0;white-space:nowrap">${escHtml(e.event)}</td>
      <td style="font-size:10px">${escHtml(e.result)}</td>
      <td style="font-size:10px;color:var(--text-faint);text-align:right">${Math.round((e.confidence || 0) * 100)}%</td>
    </tr>`
  ).join('');

  const unknownRows = unknowns.map(u =>
    `<div style="font-size:10px;padding:3px 0;border-bottom:1px solid var(--border-dim)">
      <span style="font-weight:600">${escHtml(u.field)}</span>
      <span style="margin-left:6px;color:var(--text-faint)">[${escHtml(u.severity)}]</span>
      <div style="color:var(--text-muted);font-size:9px;margin-top:1px">${escHtml(u.meaning)}</div>
    </div>`
  ).join('');
  ...
  container.innerHTML = `
    <div style="margin-bottom:10px">
      <div style="font-size:14px;font-weight:600;color:var(--text);margin-bottom:2px">${escHtml(doc.title || doc.doc_id)}</div>
      <div style="font-size:10px;color:var(--text-faint);margin-bottom:6px">${escHtml(doc.doc_id)}</div>
      <span style="font-size:12px;padding:2px 8px;border-radius:3px;background:${col};color:#fff">${escHtml((sym.currentness_label || 'unknown').replace(/_/g,' '))}</span>
      <span style="font-size:10px;color:var(--text-faint);margin-left:8px">${escHtml(sym.authority_label || '')} &middot; ${escHtml(sym.freshness_label || '')} &middot; ${escHtml(sym.conflict_label || '')}</span>
    </div>
    <div style="margin-bottom:10px">
      <div style="font-size:10px;color:var(--text-faint);margin-bottom:5px;text-transform:uppercase;letter-spacing:.05em">Dimensional pressures <span style="font-style:italic;text-transform:none">(not truth scores)</span></div>
      ${_foldScalarBar('Authority pressure', sc.authority_score)}
      ${_foldScalarBar('Freshness pressure', sc.freshness_score)}
      ${_foldScalarBar('Evidence strength', sc.evidence_strength)}
      ${_foldScalarBar('Conflict pressure', sc.conflict_pressure)}
      ${_foldScalarBar('Interpretability', sc.interpretability)}
      ${_foldScalarBar('Queryability', sc.queryability)}
      ${_foldScalarBar('Canon readiness', sc.canon_readiness)}
      ${_foldScalarBar('Drift risk', sc.drift_risk)}
      ${_foldScalarBar('Resolution confidence', sc.resolution_confidence)}
      ${_foldScalarBar('Blast radius', sc.blast_radius)}
    </div>
    ${trace.length ? `<div style="margin-bottom:10px">
      <div style="font-size:10px;color:var(--text-faint);margin-bottom:5px;text-transform:uppercase;letter-spacing:.05em">Resolver trace (${trace.length} events)</div>
      <table style="border-collapse:collapse;width:100%">${traceRows}</table>
    </div>` : ''}
    ${unknowns.length ? `<div style="margin-bottom:10px">
      <div style="font-size:10px;color:var(--text-faint);margin-bottom:4px;text-transform:uppercase;letter-spacing:.05em">Unknowns (${unknowns.length})</div>
      ${unknownRows}
    </div>` : '<div style="font-size:10px;color:var(--text-faint);margin-bottom:6px">No unknowns.</div>'}
    <div style="font-size:9px;color:var(--text-faint);margin-top:8px;border-top:1px solid var(--border-dim);padding-top:6px">${escHtml(packet.schema_version || '')} &middot; ${escHtml(packet.resolver_version || '')} &middot; ${escHtml(packet.cache_status || 'live')}</div>`;
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Dev-open mode banner: when no BOH_OPERATOR_TOKEN is set, show a subtle indicator
  try {
    const opStatus = await api('/api/operator/status');
    if (opStatus?.dev_open) {
      const banner = document.createElement('div');
      banner.id = 'dev-open-banner';
      banner.title = 'Dev-open mode: BOH_OPERATOR_TOKEN not set. All routes open. Set the env var to enforce access control.';
      banner.style.cssText = 'position:fixed;top:0;right:0;z-index:9999;padding:2px 8px;font-size:9px;background:#1a3a1a;color:#4ade80;letter-spacing:.08em;cursor:default;opacity:.85';
      banner.textContent = 'DEV-OPEN';
...
async function uploadDocuments() {
  ...
  if (bulkUploadInProgress) {
    if (status) status.textContent = 'Upload already in progress.';
    return;
  }
...
async function indexFolderFromInputPanel() {
  ...
  const res = await api('/api/index', {
    method:  'POST',
    headers: {'Content-Type': 'application/json'},
    body:    JSON.stringify({ library_root: path }),
  });
  if (res.error || res.ok === false) {
...
async function doReset(level) {
  ...
  if (level === 'state') {
    ...
    if (r.ok) {
      ...
      if (resultEl) resultEl.innerHTML = `<span style="color:var(--green-bright)">Reset + seeded ${seed.indexed_count||0}/${seed.total||0} fixtures. Running reindex...</span>`;
  ...
  if (r && r.ok) {
    if (resultEl) resultEl.innerHTML = renderResetReport(r);
    setTimeout(() => {
      document.getElementById('boh-reset-modal')?.remove();
      triggerAutoIndex(false);
    }, 1500);
  } else {
    const errMsg = await classifyProtectedError(r, 'Reset');
...
async function seedVerificationFixtures() {
  ...
  if (r && r.ok) {
    ...
    let html = `<span style="color:var(--green-bright)">Seeded ${r.indexed_count||0}/${r.total||0} fixtures under ${escHtml(r.seed_namespace||'primitive_test')}</span>`;
    if (failList.length) {
...
async function loadWorkspaceHealth() {
  ...
  const stats = r.stats || {};
  const inventoryLine = `${stats.indexed_candidates_total||0} index candidates - ${stats.primitive_fixture_files_total||0} primitive sources - ${stats.review_artifact_files_total||0} review artifacts - ${stats.quarantine_files_total||0} quarantined`;
  ...
  html += `<div class="text-faint font-sm" style="margin-bottom:6px">${escHtml(inventoryLine)}</div>`;
  ...
  el_h.innerHTML = html;
  try { updateActorStatus(); } catch (_) {}
}

async function loadActorsAuthorityPanel() {
  const box = el('st-actors-authority');
  if (!box) return;
  box.innerHTML = '<span class="spinner"></span> Loading actors...';
  const [actors, grants, ledger] = await Promise.all([
    api('/api/actors'),
    api('/api/authority/grants'),
    api('/api/ledger/recent?limit=8'),
  ]);
  const actorRows = (actors.actors || []).slice(0, 20).map(a => `
    <tr>
      <td><code>${escHtml(a.actor_id)}</code></td>
      <td>${escHtml(a.display_name || '')}</td>
      <td>${escHtml(a.actor_type || '')}</td>
      <td>${a.active ? 'active' : 'inactive'}</td>
    </tr>`).join('');
  const grantRows = (grants.grants || []).slice(0, 12).map(g => `
    <tr>
      <td><code>${escHtml(g.actor_id)}</code></td>
      <td>${escHtml(g.action)}</td>
      <td>${escHtml(g.scope_type)}${g.scope_id ? ':' + escHtml(g.scope_id) : ''}</td>
      <td>${escHtml(g.authority_level)}</td>
    </tr>`).join('');
  const ledgerRows = (ledger.events || []).map(e => `
    <tr>
      <td>${escHtml((e.created_at || '').slice(0, 19))}</td>
      <td><code>${escHtml(e.actor_id || '')}</code></td>
      <td>${escHtml(e.action)}</td>
      <td>${escHtml(e.authority_result)}</td>
      <td>${escHtml(e.target_type)}${e.target_id ? ':' + escHtml(e.target_id) : ''}</td>
    </tr>`).join('');
  box.innerHTML = `
    <div class="kv-grid" style="margin-bottom:10px">
      <div class="kv-key">Current actor</div><div class="kv-val"><code>${escHtml(getCurrentActorId())}</code></div>
      <div class="kv-key">Operator token</div><div class="kv-val">${getOperatorToken() ? 'saved for this tab' : 'not saved'}</div>
    </div>
    <div style="overflow:auto;max-height:220px;margin-bottom:12px">
      <div class="text-bright" style="margin-bottom:4px">Actors</div>
      <table class="data-table"><thead><tr><th>ID</th><th>Name</th><th>Type</th><th>Status</th></tr></thead><tbody>${actorRows || '<tr><td colspan="4">No actors</td></tr>'}</tbody></table>
    </div>
    <div style="overflow:auto;max-height:220px;margin-bottom:12px">
      <div class="text-bright" style="margin-bottom:4px">Authority grants</div>
      <table class="data-table"><thead><tr><th>Actor</th><th>Action</th><th>Scope</th><th>Level</th></tr></thead><tbody>${grantRows || '<tr><td colspan="4">No grants</td></tr>'}</tbody></table>
    </div>
    <div style="overflow:auto;max-height:220px">
      <div class="text-bright" style="margin-bottom:4px">Recent action ledger</div>
      <table class="data-table"><thead><tr><th>Time</th><th>Actor</th><th>Action</th><th>Result</th><th>Target</th></tr></thead><tbody>${ledgerRows || '<tr><td colspan="5">No ledger events</td></tr>'}</tbody></table>
    </div>`;
}
...
async function runCleanTestWorkspace() {
  ...
  if (!reset?.ok) { updateStatus(await classifyProtectedError(reset, 'Reset'), 'var(--red)'); return; }
  if (resultEl) resultEl.innerHTML = renderResetReport(reset);
  ...
  const summary = `Clean workspace ready: ${seed.indexed_count||0} fixtures indexed, ${idx.indexed||0} indexed, ${idx.skipped||0} skipped, ${idx.failed||0} failed`;
  ...
  const renderChecks = () => {
    ...
    el_v.innerHTML = `
      <div style="padding:10px 12px;border-bottom:1px solid var(--border-dim);display:flex;gap:8px;align-items:center">
        ...
        ${allPass ? '<span style="color:var(--green-bright)">Workflow verified</span>' : '<span style="color:var(--amber)">Some checks need attention</span>'}
      </div>
      ${checks.map(c => `
        <div style="display:flex;gap:10px;align-items:center;padding:7px 12px;border-bottom:1px solid var(--border-dim);font-size:11px">
          <span style="font-size:14px;flex-shrink:0">${c.pass ? 'PASS' : 'FAIL'}</span>
      ...
      ${!allPass ? `<div style="padding:10px 12px;font-size:11px;color:var(--amber)">
        Fix: save the operator token, then use Clean Test Workspace and Seed Fixtures.
      </div>` : ''}
    `;
  };

  await check('operator-server', 'Server operator token configured', async () => {
    const r = await api('/api/operator/status');
    return {pass: !!r.configured && r.protected_routes_fail_closed, note: r.configured ? r.header_name : 'BOH_OPERATOR_TOKEN unset'};
  });
  await check('operator-client', 'Client operator token saved', async () => {
    return {pass: !!getOperatorToken(), note: getOperatorToken() ? 'saved in sessionStorage' : 'not saved'};
  });
  await check('operator-missing', 'Protected route rejects missing token', async () => {
    const r = await fetch('/api/operator/check', { method: 'POST' });
    return {pass: r.status === 401 || r.status === 403, note: `HTTP ${r.status}`};
  });
  await check('operator-wrong', 'Protected route rejects wrong token', async () => {
    const r = await fetch('/api/operator/check', { method: 'POST', headers: { [OPERATOR_HEADER_NAME]: 'wrong-token' } });
    return {pass: r.status === 401 || r.status === 403, note: `HTTP ${r.status}`};
  });
  await check('operator-valid', 'Protected route accepts saved token', async () => {
    if (!getOperatorToken()) return {pass: false, note: 'no client token'};
    const preview = protectedRequestPreview('/api/operator/diagnostic', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: '{}',
    });
    const r = await bohProtectedFetch('/api/operator/diagnostic', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: '{}',
    });
    ...
    return {
      pass: r.ok && data.operator_header_received && data.actor_header_received && preview.has_operator_token && preview.has_actor_id,
      note: `HTTP ${r.status}; token=${preview.has_operator_token}; actor=${preview.has_actor_id}`,
    };
  });
  await check('db', 'DB reachable', async () => {
    const r = await api('/api/workspace/state');
    return {pass: !r.error, note: r.error || `${r.stats?.doc_count || 0} docs`};
  });
  await check('library', 'Library root exists', async () => {
    const r = await api('/api/workspace/state');
    return {pass: !!r.stats?.library_exists, note: r.stats?.library_root || 'unknown'};
  });
  await check('fs-boundary', 'Filesystem boundary rejects escape', async () => {
    const r = await api('/api/workspace/boundary-check');
    return {pass: r.escape_rejected === true, note: r.escape_rejected ? 'escape rejected' : 'escape accepted'};
  });
  await check('import-staging', 'Import staging exists', async () => {
    const r = await api('/api/workspace/state');
    return {pass: !!r.stats?.library_exists, note: 'imports staged under BOH_LIBRARY'};
  });
  await check('fts', 'FTS/search reachable', async () => {
    const r = await api('/api/search?q=primitive&limit=1');
    return {pass: !r.error, note: r.error || 'search route responded'};
  });
  await check('bulk-import', 'Bulk Import reachable', async () => {
    const r = await api('/api/input/recent?limit=1');
    return {pass: !r.error, note: r.error || 'input routes reachable'};
  });
  await check('docs', 'Docs list reachable', async () => {
    const r = await api('/api/docs?per_page=1');
    return {pass: !r.error, note: r.error || `${r.total || 0} docs`};
  });
  await check('fixtures', 'Primitive fixtures present/indexed', async () => {
    const r = await api('/api/docs?per_page=200&project=Primitive%20Test');
    const n = (r.docs || []).filter(d => String(d.path || '').startsWith('fixtures/primitive_test/')).length;
    return {pass: n >= 6, note: `${n} primitive docs indexed`};
  });
  await check('inventory-counts', 'Library inventory counts visible', async () => {
    const r = await api('/api/workspace/state');
    const s = r.stats || {};
    const note = `${s.library_file_count || 0} files; ${s.indexed_candidates_total || 0} candidates; ${s.primitive_fixture_files_total || 0} primitive sources; ${s.review_artifact_files_total || 0} review artifacts; ${s.quarantine_files_total || 0} quarantine`;
    return {pass: !r.error, note};
  });
  await check('quarantine-excluded', 'Quarantine excluded from autoindex', async () => {
    const r = await api('/api/autoindex/report');
    const manifest = r.manifest || [];
    const leaking = manifest.filter(row => row.classification === 'quarantine' && row.action !== 'skipped_excluded');
    const skipped = manifest.filter(row => row.classification === 'quarantine' && row.action === 'skipped_excluded');
    return {pass: !r.error && leaking.length === 0, note: leaking.length ? `${leaking.length} quarantine files would index` : `${skipped.length} quarantine files skipped`};
  });
  await check('review-artifacts-excluded', 'Review artifacts excluded from autoindex', async () => {
    const r = await api('/api/autoindex/report');
    const manifest = r.manifest || [];
    const leaking = manifest.filter(row => row.classification === 'review_artifact' && row.action !== 'skipped_excluded');
    const skipped = manifest.filter(row => row.classification === 'review_artifact' && row.action === 'skipped_excluded');
    return {pass: !r.error && leaking.length === 0, note: leaking.length ? `${leaking.length} review artifacts would index` : `${skipped.length} review artifacts skipped`};
  });
  await check('idx', 'Last reindex reachable', async () => {
    const r = await api('/api/autoindex/status');
    const failed = r.last_failed || 0;
    return {pass: !r.error && failed === 0, note: r.error || `${failed} failures`};
  });
  await check('ollama', 'Ollama status reachable', async () => {
    const r = await api('/api/ollama/health');
    return {pass: !r.error, note: r.error || (r.ok ? 'enabled/reachable' : 'disabled or unavailable')};
  });
...
async function uploadDocumentsFromInput(inputEl) {
  ...
  if (bulkUploadInProgress) {
    if (statusEl) statusEl.textContent = 'Upload already in progress.';
    return;
  }
  bulkUploadInProgress = true;
  const fd = new FormData();
  ...
  try {
    const r = await fetch('/api/input/upload', withOperatorHeaders('/api/input/upload', {
      method: 'POST',
      body: fd,
    }));
    const data = await r.json();
    ...
    const ok = saved.filter(x => x.indexed).length;
    const skipped = saved.filter(x => x.skipped).length;
    const fail = saved.filter(x => !x.indexed && !x.skipped).length + rejected.length;
    if (statusEl) statusEl.innerHTML = `<span class="text-green">${ok} indexed</span>${skipped ? ` <span class="text-amber">(${skipped} unchanged)</span>` : ''}${fail ? ` <span class="text-red">(${fail} failed)</span>` : ''}`;
  } catch (e) {
...
function openFolderPicker() {
  if (bulkUploadInProgress) {
    const status = el('index-folder-status') || el('folder-picker-status') || el('upload-status-new') || el('upload-status');
    if (status) status.textContent = 'Upload already in progress.';
    return;
  }
  ...
  input.addEventListener('change', async () => {
    ...
    const supported = ['.md', '.markdown', '.mdx', '.txt', '.rst', '.log', '.html', '.htm', '.json', '.jsonl', '.yaml', '.yml', '.csv', '.tsv', '.toml', '.ini', '.cfg', '.conf', '.properties', '.env.example', '.xml', '.tex', '.bib', '.ipynb', '.docx'];
    ...
    for (let i = 0; i < filtered.length; i += batchSize) {
      ...
      try {
        ...
        const saved = data.saved || [];
        const rejected = data.rejected || [];
        indexed += saved.filter(x => x.indexed).length;
        const skipped = saved.filter(x => x.skipped).length;
        failed  += saved.filter(x => !x.indexed && !x.skipped).length + rejected.length;
        if (!window._bohFolderSkipped) window._bohFolderSkipped = 0;
        window._bohFolderSkipped += skipped;
...
Object.assign(window, {
  ...
  loadActorsAuthorityPanel: (typeof loadActorsAuthorityPanel === 'function' ? loadActorsAuthorityPanel : undefined),
  ...
  saveCurrentActor: (typeof saveCurrentActor === 'function' ? saveCurrentActor : undefined),
  saveOperatorToken: (typeof saveOperatorToken === 'function' ? saveOperatorToken : undefined),
  clearOperatorToken: (typeof clearOperatorToken === 'function' ? clearOperatorToken : undefined),
...
async function loadTraceLedger() {
  const body = el('trace-ledger-body');
  if (!body) return;
  const posture = el('trace-ledger-posture')?.value || '';
  body.innerHTML = tableStatusRow(8, '<span class="spinner"></span> Loading gate results…');
  const qs = posture ? `?limit=200&posture=${encodeURIComponent(posture)}` : '?limit=200';
  const r = await api(`/api/trace/gate-results${qs}`);
  const rows = r.gate_results || [];
  const countEl = el('trace-ledger-count');
  if (countEl) countEl.textContent = String(rows.length);
  renderTable('trace-ledger-body', rows, g => `
    <tr>
      <td>${escHtml(g.posture || '—')}</td>
      <td>${escHtml(g.operation || '—')}</td>
      <td>${escHtml(g.actor_id || '—')}</td>
      <td title="${escHtml((g.blocking_reasons || []).join(', '))}">${(g.blocking_reasons || []).length}</td>
      <td title="${escHtml((g.warning_reasons || []).join(', '))}">${(g.warning_reasons || []).length}</td>
      <td>${(g.allowed_context_refs || []).length}</td>
      <td>${(g.withheld_context_refs || []).length}</td>
      <td title="${escHtml(g.gate_result_id || '')}"><code>${escHtml((g.gate_result_id || '').slice(0, 16))}</code></td>
    </tr>`, { colspan: 8, empty: 'No gate results recorded yet.' });
}

async function loadResidenceMap() {
  const body = el('residence-map-body');
  if (!body) return;
  const status = el('residence-map-status')?.value || '';
  body.innerHTML = tableStatusRow(6, '<span class="spinner"></span> Loading residence rows…');
  const qs = status ? `?limit=200&status=${encodeURIComponent(status)}` : '?limit=200';
  const r = await api(`/api/residence/map${qs}`);
  const rows = r.residences || [];
  const countEl = el('residence-map-count');
  if (countEl) countEl.textContent = String(rows.length);
  renderTable('residence-map-body', rows, m => `
    <tr>
      <td title="${escHtml(m.original_ref || '')}"><code>${escHtml((m.original_ref || '').slice(0, 20))}</code></td>
      <td title="${escHtml(m.current_ref || '')}"><code>${escHtml((m.current_ref || '').slice(0, 20))}</code></td>
      <td>${escHtml(m.current_location || '—')}</td>
      <td>${escHtml(m.status || '—')}</td>
      <td title="${escHtml(m.human_readable_locator || '')}">${escHtml((m.human_readable_locator || '').slice(0, 40))}</td>
      <td title="${escHtml(m.reason || '')}">${escHtml((m.reason || '').slice(0, 40))}</td>
    </tr>`, { colspan: 6, empty: 'No residence rows recorded yet.' });
}

function loadContextPackBuilder() {
  // Static form; nothing to fetch on entry. Assemble is user-triggered.
}

async function assembleContextPack() {
  const statusEl = el('cpb-status');
  const resultEl = el('cpb-result');
  const postureEl = el('cpb-posture');
  if (!resultEl) return;
  let packs;
  try {
    packs = JSON.parse(el('cpb-packs')?.value || '[]');
    if (!Array.isArray(packs)) throw new Error('candidate_packs must be a JSON array');
  } catch (e) {
    if (statusEl) statusEl.textContent = 'Invalid JSON: ' + e.message;
    return;
  }
  if (statusEl) statusEl.textContent = 'Assembling…';
  const r = await api('/api/context-pack/assemble', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      query: el('cpb-query')?.value || '',
      operation: el('cpb-operation')?.value || 'answer_context',
      actor: el('cpb-actor')?.value || null,
      mode: el('cpb-mode')?.value || 'exploration',
      candidate_packs: packs,
    }),
  });
  if (statusEl) statusEl.textContent = '';
  if (postureEl) postureEl.textContent = r.posture || '—';
  const sections = r.sections || {};
  const labels = ['canon', 'evidence', 'interpretation', 'conflict', 'open_questions'];
  const secHtml = labels.map(label => {
    const entries = sections[label] || [];
    const items = entries.length
      ? entries.map(e => `<li><code>${escHtml(e.ref || '')}</code> <span class="text-faint">${escHtml(e.title || '')}</span></li>`).join('')
      : '<li class="text-faint">—</li>';
    return `<div style="margin-bottom:8px"><strong>${escHtml(label)}</strong> <span class="count">${entries.length}</span><ul style="margin:4px 0 0;padding-left:18px;font-size:12px">${items}</ul></div>`;
  }).join('');
  const withheld = r.withheld || {};
  const reasons = (withheld.reasons || []);
  const missing = (r.missing_planes || []);
  resultEl.innerHTML = `
    <div style="margin-bottom:8px;font-size:11px" class="text-faint">
      assembled_pack_id: <code>${escHtml(r.assembled_pack_id || '')}</code> · canon_eligible: ${String(r.canon_eligible)}
    </div>
    ${secHtml}
    <div style="margin-top:8px"><strong>Withheld</strong>${reasons.length ? '<ul style="margin:4px 0 0;padding-left:18px;font-size:12px">' + reasons.map(x => `<li>${escHtml(String(x))}</li>`).join('') + '</ul>' : ' <span class="text-faint">none</span>'}</div>
    <div style="margin-top:8px"><strong>Missing planes</strong> ${missing.length ? escHtml(missing.join(', ')) : '<span class="text-faint">none</span>'}</div>`;
}

async function loadInbox() {
  const body = el('inbox-cap-body');
  if (!body) return;
  const lc = el('inbox-filter-lifecycle')?.value.trim() || '';
  const lane = el('inbox-filter-lane')?.value.trim() || '';
  const batch = el('inbox-filter-batch')?.value.trim() || '';
  const qs = new URLSearchParams({ limit: '200' });
  if (lc) qs.set('lifecycle_state', lc);
  if (lane) qs.set('safety_lane', lane);
  if (batch) qs.set('batch_id', batch);
  body.innerHTML = tableStatusRow(9, '<span class="spinner"></span> Loading capabilities…');
  const r = await api(`/api/intake/capabilities?${qs.toString()}`);
  if (r.error) { body.innerHTML = tableStatusRow(9, escHtml(r.error)); return; }
  const items = r.items || [];
  const countEl = el('inbox-cap-count');
  if (countEl) countEl.textContent = `${items.length} / ${r.total ?? items.length}`;
  renderTable('inbox-cap-body', items, c => `
    <tr>
      <td title="${escHtml(c.source_ref || '')}"><code>${escHtml((c.source_ref || '').slice(-40))}</code></td>
      <td>${escHtml(c.lifecycle_state || '—')}</td>
      <td>${escHtml(c.safety_lane || '—')}</td>
      <td>${escHtml(c.batch_id || '—')}</td>
      <td>${c.preservable ? '✓' : '·'}</td>
      <td>${c.normalizable ? '✓' : '·'}</td>
      <td>${c.queryable ? '✓' : '·'}</td>
      <td>${escHtml(c.required_adapter || '—')}</td>
      <td title="${escHtml(c.failure_reason || '')}">${escHtml((c.failure_reason || '').slice(0, 40))}</td>
    </tr>`, { colspan: 9, empty: 'No intake capabilities recorded yet.' });

  const lanesEl = el('inbox-lanes');
  if (lanesEl) {
    const lr = await api('/api/intake/safety-lanes');
    if (lr.error) { lanesEl.innerHTML = `<div class="text-faint">${escHtml(lr.error)}</div>`; }
    else {
      const lanes = lr.lanes || {};
      const keys = Object.keys(lanes);
      lanesEl.innerHTML = keys.length
        ? `<div class="font-sm">${keys.map(k => `<span class="badge" style="margin-right:6px">${escHtml(k)}: ${escHtml(String(lanes[k]))}</span>`).join('')} <span class="text-faint">total ${escHtml(String(lr.total ?? 0))}</span></div>`
        : '<div class="text-faint">No capabilities yet.</div>';
    }
  }

  const adaptersEl = el('inbox-adapters');
  if (adaptersEl) {
    const ar = await api('/api/intake/adapters');
    if (ar.error) { adaptersEl.innerHTML = `<div class="text-faint">${escHtml(ar.error)}</div>`; }
    else {
      const cov = ar.coverage_report || {};
      const entries = Object.entries(cov);
      adaptersEl.innerHTML = entries.length
        ? `<div class="font-sm" style="white-space:pre-wrap">${escHtml(JSON.stringify(cov, null, 2))}</div>`
        : '<div class="text-faint">No adapter coverage reported.</div>';
    }
  }
}
...
async function loadQuarantine() {
  ...
  if (!body) return;
  body.innerHTML = tableStatusRow(6, '<span class="spinner"></span> Loading quarantine records…');
  const r = await api('/api/intake/quarantine?limit=200');
  if (r.error) { body.innerHTML = tableStatusRow(6, escHtml(r.error)); return; }
  const items = r.items || [];
  const countEl = el('quarantine-count');
  if (countEl) countEl.textContent = `${items.length} / ${r.total ?? items.length}`;
  renderTable('quarantine-body', items, q => `
    <tr>
      <td title="${escHtml(q.quarantine_record_id || '')}"><code>${escHtml((q.quarantine_record_id || '').slice(0, 20))}</code></td>
      <td title="${escHtml(q.intake_capability_id || '')}"><code>${escHtml((q.intake_capability_id || '').slice(0, 20))}</code></td>
      <td>${escHtml(q.quarantine_category || '—')}</td>
      <td title="${escHtml(q.quarantine_reason || '')}">${escHtml((q.quarantine_reason || '').slice(0, 60))}</td>
      <td>${q.review_required ? 'yes' : 'no'}</td>
      <td>${escHtml(q.created_at || '—')}</td>
    </tr>`, { colspan: 6, empty: 'No quarantine records.' });
}

async function runIntakeBatch() {
  const statusEl = el('inbox-run-status');
  const source = el('inbox-run-source')?.value.trim() || '';
  const batch = el('inbox-run-batch')?.value.trim() || '';
  if (!source || !batch) { if (statusEl) statusEl.textContent = 'source_ref and batch_id are required.'; return; }
  if (statusEl) statusEl.textContent = 'Running…';
  const r = await api('/api/intake/run', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ source_ref: source, batch_id: batch }),
  });
  if (r.error) {
    if (statusEl) statusEl.textContent = r.error;
    return;
  }
  if (statusEl) statusEl.textContent = `status=${r.status || 'complete'} · capability=${r.intake_capability_id || '—'} · route=${r.route || '—'}`;
  loadInbox();
}

async function loadAuthorityLedger() {
  // Promotion ledger
  const pbody = el('al-promo-body');
  if (pbody) {
    const target = el('al-promo-target')?.value.trim() || '';
    const pqs = new URLSearchParams({ limit: '200' });
    if (target) pqs.set('target_id', target);
    pbody.innerHTML = tableStatusRow(7, '<span class="spinner"></span> Loading promotions…');
    const pr = await api(`/api/authority/promotions?${pqs.toString()}`);
    if (pr.error) {
      pbody.innerHTML = tableStatusRow(7, escHtml(pr.error));
    } else {
      const promos = pr.promotions || [];
      const pc = el('al-promo-count');
      if (pc) pc.textContent = String(promos.length);
      renderTable('al-promo-body', promos, p => `
        <tr>
          <td title="${escHtml(p.promotion_id || '')}"><code>${escHtml((p.promotion_id || '').slice(0, 20))}</code></td>
          <td>${escHtml(p.old_authority || '—')} → ${escHtml(p.new_authority || '—')}</td>
          <td title="${escHtml(p.promotion_reason || '')}">${escHtml((p.promotion_reason || '').slice(0, 50))}</td>
          <td>${escHtml(p.approved_by || '—')}</td>
          <td title="${escHtml(p.target_id || '')}">${escHtml((p.target_id || '—'))}</td>
          <td>${escHtml(p.promotion_timestamp || '—')}</td>
          <td title="${escHtml(p.lineage_ref || '')}">${escHtml((p.lineage_ref || '—').slice(0, 20))}</td>
        </tr>`, { colspan: 7, empty: 'No authority promotions recorded yet.' });
    }
  }

  // Resolution log
  const lbody = el('al-log-body');
  if (lbody) {
    const actor = el('al-log-actor')?.value.trim() || '';
    const ltarget = el('al-log-target')?.value.trim() || '';
    const authorizedOnly = el('al-log-authorized')?.checked || false;
    const lqs = new URLSearchParams({ limit: '200' });
    if (actor) lqs.set('actor_id', actor);
    if (ltarget) lqs.set('target_id', ltarget);
    if (authorizedOnly) lqs.set('authorized_only', 'true');
    lbody.innerHTML = tableStatusRow(7, '<span class="spinner"></span> Loading resolution log…');
    const lr = await api(`/api/authority/log?${lqs.toString()}`);
    if (lr.error) {
      lbody.innerHTML = tableStatusRow(7, escHtml(lr.error));
    } else {
      const log = lr.log || [];
      const lc = el('al-log-count');
      if (lc) lc.textContent = String(log.length);
      renderTable('al-log-body', log, e => `
        <tr>
          <td>${escHtml(e.timestamp || '—')}</td>
          <td>${escHtml(e.actor_id || '—')}</td>
          <td>${escHtml(e.actor_role || '—')}${e.actor_team ? ' / ' + escHtml(e.actor_team) : ''}</td>
          <td title="${escHtml(e.target_id || '')}">${escHtml((e.target_id || '—'))}</td>
          <td>${escHtml(e.required_authority || '—')}</td>
          <td>${e.authorization_result ? '<span style="color:var(--green-bright)">authorized</span>' : '<span style="color:var(--red)">rejected</span>'}</td>
          <td title="${escHtml(e.failure_reason || '')}">${escHtml((e.failure_reason || '').slice(0, 50))}</td>
        </tr>`, { colspan: 7, empty: 'No resolution log entries.' });
    }
  }
}

async function loadReviewQueue() {
  const body = el('review-queue-body');
  if (!body) return;
  const status = el('review-queue-status')?.value ?? 'proposed';
  body.innerHTML = '<div class="text-faint font-sm"><span class="spinner"></span> Loading proposals…</div>';
  const qs = status ? `?limit=200&status=${encodeURIComponent(status)}` : '?limit=200';
  const r = await api(`/api/review-queue/proposals${qs}`);
  const rows = r.proposals || [];
  const countEl = el('review-queue-count');
  if (countEl) countEl.textContent = String(rows.length);
  if (!rows.length) {
    body.innerHTML = '<div class="text-faint font-sm">No patch proposals for this filter.</div>';
    return;
  }
  body.innerHTML = rows.map(p => {
    const pid = p.patch_id || '';
    const isSchema = p.proposal_type === 'schema_patch';
    const open = p.status === 'proposed';
    const controls = open ? `
      <div style="margin-top:8px;padding:8px;border-top:1px solid var(--border-dim);display:grid;gap:6px">
        <input id="rq-co-${pid}" placeholder="changed_objects (comma-separated)" style="font-size:11px;padding:3px 6px">
        <input id="rq-fx-${pid}" placeholder="regression_fixture_refs (comma-separated${isSchema ? '; optional for schema_patch' : '; required'})" style="font-size:11px;padding:3px 6px">
        <input id="rq-note-${pid}" placeholder="migration / review note" style="font-size:11px;padding:3px 6px">
        <div style="display:flex;gap:6px;align-items:center">
          <button class="btn btn-sm" onclick="approveProposal('${pid}')">Approve</button>
          <button class="btn btn-ghost btn-sm" onclick="rejectProposal('${pid}')">Reject</button>
          <span id="rq-result-${pid}" class="font-sm"></span>
        </div>
      </div>` : '';
    return `
      <div class="card" style="margin-bottom:8px;padding:10px;border:1px solid var(--border-dim);border-radius:6px">
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <strong>${escHtml(p.proposal_type || '—')}</strong>
          <span class="badge">${escHtml(p.status || '—')}</span>
          <span class="font-sm text-faint">blast: ${escHtml(p.blast_radius || '—')} · review by: ${escHtml(p.requires_review_by || '—')}${p.forbidden_auto_apply ? ' · no-auto-apply' : ''}</span>
          <code class="font-sm ml-auto" title="${escHtml(pid)}">${escHtml(pid.slice(0, 16))}</code>
        </div>
        <div class="font-sm" style="margin-top:4px">${escHtml(p.proposed_change || '')}</div>
        ${(p.evidence_refs || []).length ? `<div class="font-sm text-faint" style="margin-top:2px">evidence: ${escHtml((p.evidence_refs || []).join(', '))}</div>` : ''}
        ${controls}
      </div>`;
  }).join('');
}
...
async function approveProposal(pid) {
  const out = el(`rq-result-${pid}`);
  const changed = _rqCsv(`rq-co-${pid}`);
  const fixtures = _rqCsv(`rq-fx-${pid}`);
  const note = el(`rq-note-${pid}`)?.value?.trim() || '';
  if (!changed.length) { if (out) out.innerHTML = '<span class="text-red">changed_objects is required.</span>'; return; }
  if (out) out.innerHTML = '<span class="spinner"></span>';
  const resp = await bohProtectedFetch(`/api/review-queue/proposals/${encodeURIComponent(pid)}/approve`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ approved_by: 'operator', changed_objects: changed, migration_note: note, regression_fixture_refs: fixtures }),
  });
  const data = await resp.json().catch(() => ({}));
  if (!resp.ok) { if (out) out.innerHTML = `<span class="text-red">${escHtml(data.detail || ('HTTP ' + resp.status))}</span>`; return; }
  loadReviewQueue();
}
...
async function rejectProposal(pid) {
  const out = el(`rq-result-${pid}`);
  const note = el(`rq-note-${pid}`)?.value?.trim() || '';
  if (!note) { if (out) out.innerHTML = '<span class="text-red">A note is required to reject.</span>'; return; }
  if (out) out.innerHTML = '<span class="spinner"></span>';
  const resp = await bohProtectedFetch(`/api/review-queue/proposals/${encodeURIComponent(pid)}/reject`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
function clearVisualizationSelection() {
  ...
  if (fold) { fold.innerHTML = ''; fold.classList.add('hidden'); }
  const snap = el('reader-fold-snapshot');
  if (snap) { snap.innerHTML = ''; snap.classList.add('hidden'); }
...
function _renderVariablePanel(nodes, edges, sidebar, diagnostics) {
  ...
  ${!hasData ? `<div class="viz-gap-notice">⚠ No Confidence State data found. Load the <button class="btn btn-ghost btn-sm" onclick="loadDemoProject()">Demo</button> to see d/m/q/c values.</div>` : ''}
...
function _renderConstitutionalPanel(nodes, edges, sidebar, diagnostics) {
  ...
  ${_vizDiagnosticBanner(diagnostics)}
  ${!hasEpistemic?`<div class="viz-gap-notice">ℹ No Confidence State metadata. All nodes in raw_imported lane. <button class="btn btn-ghost btn-sm" onclick="loadDemoProject()">Load Demo</button></div>`:''}
  <div class="viz-grammar-grid">
...
async function backfillPlaneCards() {
  ...
  const data = await api('/api/planes/backfill', { method: 'POST' });
