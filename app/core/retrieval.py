...
def build_chunks(doc: dict, body: str, frontmatter_text: str = "",
    ...
    def add_chunk(text: str, heading_path: str, chunk_type: str, start_char: int, end_char: int):
        nonlocal index
        clean = text.strip()
        if not clean:
            return
        byte_start = len(body[:start_char].encode("utf-8"))
        byte_end = len(body[:end_char].encode("utf-8"))
        item = {
            ...
            "byte_end": byte_end,
            "token_start": _token_offset(body, byte_start),
            "token_end": _token_offset(body, byte_end),
    ...
    if frontmatter_text:
        add_chunk(frontmatter_text, "frontmatter", "frontmatter", 0, 0)
    ...
    def flush(end_pos: int):
        nonlocal buf, buf_start, buf_type
        if buf:
            add_chunk("\n".join(buf), " > ".join(heading_stack), buf_type, buf_start, end_pos)
            buf = []
            buf_type = "body"
    ...
    for raw_line in body.splitlines(keepends=True):
        ...
        if m:
            ...
            add_chunk(line, " > ".join(heading_stack), "heading", line_start, cursor)
            continue
        typ = _line_type(line)
        if not buf:
