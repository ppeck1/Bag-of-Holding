"""app/core/canon.py: Deterministic canon resolution engine for Bag of Holding v2.

Moved from canon.py (v0P). Logic unchanged.
Patch v1.1: A1.3 topic LIKE query, A1.4 topic boost, B1 plane_scope filter, B2 collision fix.

Ontology label: BOH_CANON_v3.7
Score formula: See docs/math_authority.md §1, §2, §3.
"""
...
def resolve_canon(topic: str = None, plane_scope: str = None) -> dict:
    ...
    A1.4: Topic boost (+20) applied when topic param is active.
    B1: Filters by plane_scope overlap when plane_scope param provided.

    See math_authority.md §1–§3 for all formulas.
    ...
    for doc in docs:
        ...
        if topic:
            ...
            s += 20 * topic_score  # see math_authority.md §2
...
def _ensure_conflict(conflict_type: str, doc_ids: str, term: str, plane_path: str):
    """Insert conflict only if same (conflict_type, doc_ids, term) not already present."""
