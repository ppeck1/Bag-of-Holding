...
def _metadata_completeness(n: dict) -> float:
    # Delegates to the neutral scalar_metrics module (single source of truth,
    # shared with the Current Fold View canon-geometry path). Output-preserving.
    return scalar_metrics.metadata_completeness(n)
...
def _metrics(n: dict, edge_count: int, cross_unapproved: int) -> dict[str, float]:
    ...
    # Viability and projection-loss formulas live in the neutral scalar_metrics
    # module (shared with the Current Fold View canon-geometry path). The graph
    # output field names (viability_margin / projection_loss) are unchanged.
    viability = scalar_metrics.omega_viability(
        authority_score, conflicts, expired, uncertain, completeness
    )
    ...
    projection_loss  = scalar_metrics.mismatch_gradient(completeness)
...
def _sidebar_variable(nodes: list) -> dict:
    ...
    return {
        ...
        "gap": (None if has_data
                else "No Daenary epistemic metadata found. Load Demo Project "
    ...
    if mode == "variable":
        ...
        if not any(n.get("epistemic_q") is not None for n in nodes):
            out.append({"severity": "info", "code": "NO_EPISTEMIC_DATA",
                        "message": "No Daenary epistemic_state found. Load Demo Project "
                                   "to see real d/m/q/c values. Showing sentinel cluster."})
    elif mode == "constraint":
        if not any(n.get("epistemic_q") is not None for n in nodes):
            out.append({"severity": "info", "code": "NO_VIABILITY_DATA",
                        "message": "No epistemic q/c data. Viability surface is empty. "
                                   "Load Demo Project to see meaningful layout."})
        ...
        if not any(n.get("epistemic_d") is not None or n.get("epistemic_q") is not None for n in nodes):
            out.append({"severity": "info", "code": "NO_CUSTODIAN_DATA",
                        "message": "No Daenary epistemic metadata. All nodes in raw_imported lane. "
                                   "Load Demo Project to see governance topology."})
    return out


def get_projection_graph(
    mode: str = "web",
    max_nodes: int = 300,
    ...
    base      = get_graph_data(max_nodes=max_nodes, library_id=library_id)
