...
def save_upload(filename: str, content_bytes: bytes,
    ...
    if preferred_file.exists():
        ...
        existing_text = existing_bytes.decode("utf-8", errors="replace") if ext in (".md", ".txt", ".markdown", ".rst") else ""
        same_text_content = bool(write_text is not None and existing_text.replace("\r\n", "\n") == write_text.replace("\r\n", "\n"))
        same_generated_source = bool(source_hash and f"source_hash: {source_hash}" in existing_text)
