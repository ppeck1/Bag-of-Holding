...
def _build_node_scale_actions(base_packet: dict[str, Any]) -> list[FoldScaleAction]:
    ...
    that consume them are Phase 7b. Reads only the packet -- no DB access, no mutation.
    """
    summary = base_packet.get("summary", {})
    ...
    # --- domain axis: kept disabled; doc→domain linkage not yet implemented ---
    actions.append(FoldScaleAction(
        label="Roll up to domain",
        target_scale="cluster",
        target_axis="domain",
        allowed=False,
        reason="doc→domain linkage not yet implemented",
    ))
