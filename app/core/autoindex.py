...
EXCLUDED_DIR_NAMES = {".boh_quarantine", "__pycache__", ".pytest_cache"}
SYSTEM_DIR_NAMES = {"System Volume Information", "$RECYCLE.BIN"}
GENERATED_REVIEW_DIR_NAMES = {"review_artifacts", "reviews", ".reviews"}
...
def classify_relative_path(rel_path: str) -> str:
    rel = rel_path.replace("\\", "/").strip("/")
    parts = rel.split("/") if rel else []
    name = parts[-1] if parts else ""
    if ".boh_quarantine" in parts:
        return "quarantine"
    if name.endswith(".review.json") or name.endswith(".review.md") or any(p in GENERATED_REVIEW_DIR_NAMES for p in parts):
        return "review_artifact"
    if parts and parts[0] == "fixtures" and len(parts) > 1 and parts[1] == "primitive_test":
        return "fixture"
    if parts and parts[0] == "imports":
        return "import"
    if parts and parts[0] == "scratch":
        return "scratch"
    if parts and parts[0].startswith("demo"):
        return "demo"
    return "unknown"


def exclusion_reason(path: Path, root: Path, include_scratch: bool = False) -> str | None:
    rel = _rel(path, root)
    parts = rel.split("/")
    name = path.name
    if any(part in EXCLUDED_DIR_NAMES for part in parts):
        return "excluded_folder"
    if any(part.startswith(".") or part in SYSTEM_DIR_NAMES for part in parts):
        return "hidden_or_system_folder"
    if any(part in GENERATED_REVIEW_DIR_NAMES for part in parts):
        return "generated_review_artifact_folder"
    if name.endswith(".review.json") or name.endswith(".review.md"):
        return "generated_review_artifact"
    if "scratch" in parts and not include_scratch:
        return "scratch_excluded_by_default"
    return None


def discover_manifest(library_root: str, max_files: int = 5000,
                      include_scratch: bool = False) -> list[dict]:
    """Discover supported files and classify index action before indexing."""
    ...
    for p in root.rglob("*"):
        ...
        if not p.is_file() or p.suffix.lower() not in SUPPORTED_EXTENSIONS:
            continue
        rel = _rel(p, root)
        reason = exclusion_reason(p, root, include_scratch=include_scratch)
        manifest.append({
            "relative_path": rel,
            "classification": classify_relative_path(rel),
            "action": "skipped_excluded" if reason else "candidate",
...
def scan_library(library_root: str, max_files: int = 5000, include_scratch: bool = False) -> list[str]:
    """Recursively scan library_root for supported source files."""
    root = Path(library_root).expanduser().resolve()
    manifest = discover_manifest(str(root), max_files=max_files, include_scratch=include_scratch)
    return [str(root / m["relative_path"]) for m in manifest if m["action"] == "candidate"]
...
def run_auto_index(
    ...
    manifest = discover_manifest(root, max_files=max_files, include_scratch=include_scratch)
    paths   = [str(Path(root).resolve() / m["relative_path"]) for m in manifest if m["action"] == "candidate"]
    ...
    post_manifest = discover_manifest(root, max_files=max_files, include_scratch=include_scratch)
    logged_paths = {e.get("relative_path") or e.get("path") for e in log}
    for m in post_manifest:
        if m["action"] == "skipped_excluded" and m["relative_path"] not in logged_paths:
            log.append({"path": m["relative_path"], "status": "skipped_excluded", **m})
    ...
    return {
        ...
        "log": log[-20:],
        "manifest": log,
        "indexed_candidates_total": len(paths),
    }
