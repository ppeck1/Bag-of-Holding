...
def ingest_snapshot_export(path_to_json: str) -> dict:
    ...
    """
    try:
        requested = Path(path_to_json)
        if requested.is_absolute():
            root = get_library_root()
            snapshot_path = requested.resolve()
            snapshot_path.relative_to(root)
        else:
            snapshot_path = resolve_under_library(path_to_json)
    except (PathBoundaryViolation, ValueError) as exc:
    ...
    try:
        for file_entry in files:
            ...
            try:
                path = normalize_library_relative_path(meta.get("path") or f"snapshot/{doc_id}.md")
            except PathBoundaryViolation as exc:
                skipped.append({"reason": "unsafe_path", "doc_id": doc_id, "error": str(exc)})
                continue
