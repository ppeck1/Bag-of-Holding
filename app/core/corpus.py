...
CLASS_PROMOTED_INTAKE = "CORPUS_CLASS:PROMOTED_INTAKE"  # WO-2 advisory bridge docs
...
def classify(doc: dict) -> str:
    ...
    # 0. Promoted intake derivative (WO-2) — frontmatter-declared. This rule guarantees that
    #    RE-indexing a promoted file (autoindex, bulk scan, or an orphan from an interrupted
    #    promotion) can never strip the exposure marker the shared predicate keys on.
    if doc_type == "promoted_intake" or (doc.get("document_class") or "") == "promoted_intake":
        return CLASS_PROMOTED_INTAKE
