...
def save_draft_to_disk(doc_id: str, library_root: str,
    ...
    existing = db.fetchone("SELECT status, path, corpus_class FROM docs WHERE doc_id = ?", (doc_id,))
    ...
    # ── Enforcement 1b: promoted-intake managed documents (WO-2) ─────────────────
    # Gate-independent: the read-exposure gate may make these visible, but ordinary
    # authoring saves would break the artifact → file → doc → promotion-ledger chain.
    # Only governed promote()/demote()/supersession may mutate them.
    from app.core import promoted_exposure
    if existing and existing.get("corpus_class") == promoted_exposure.PROMOTED_CORPUS_CLASS:
        return {"error": "This is a promoted intake managed document; ordinary editor saves "
                         "are not permitted. Use governed demotion/supersession instead.",
                promoted_exposure.MUTATION_BLOCK_REASON: True}
