...
def search(
    ...
    # syntax error (boh_retrieval_fts_query_hyphen_hardening_v0_1). Identical semantics for
    # plain words; hyphenated tokens match as their tokenizer sub-token phrase.
    import re as _re
    terms = _re.findall(r"[A-Za-z0-9_][A-Za-z0-9_\-]*", query)[:12]
    if not terms:
        return []
