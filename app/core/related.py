...
def get_graph_data(max_nodes: int = 200, library_id: str | None = None) -> dict:
    ...
    docs = db.fetchall(
        ...
        FROM docs
        WHERE (corpus_class IS NULL OR corpus_class <> 'CORPUS_CLASS:PROMOTED_INTAKE')
        """
        + library_clause
        + """
...
def get_neighborhood(doc_id: str, depth: int = 1, limit: int = 40) -> dict:
    ...
    # WO-2: promoted intake docs are excluded from graph surfaces unless the env gate is open.
