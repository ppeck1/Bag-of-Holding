...
ALLOWED_SHELL_COMMANDS = {
    c.strip().lower()
    for c in os.environ.get("BOH_EXEC_ALLOWED_COMMANDS", "python,python3,py,echo,ls,pwd").split(",")
    if c.strip()
}
...
def _check_shell_safety(code: str) -> Optional[str]:
    ...
    try:
        argv = shlex.split(code, posix=False)
    except ValueError as exc:
        return f"Execution denied: invalid shell command: {exc}"
    if not argv:
        return "Execution denied: empty shell command"
    command = Path(argv[0]).name.lower()
    if command not in ALLOWED_SHELL_COMMANDS:
        return f"Execution denied: command {command!r} is not in the allowlist."
