...
1. /v2 browser-interaction regression (Layer 3 checklist, internal record) — needs browser
   tooling (playwright) approval or a human session. Gates classic unmount.
2. Responsive / accessibility pass — narrow-screen drawers below 1280px (CSS + minimal JS
   for drawer state / focus / keyboard if required).
3. Classic UI unmount + archive — after (1) passes.
