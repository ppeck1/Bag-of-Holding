"""conflicts.py: Conflict detection for Bag of Holding v0P.
...
def _detect_definition_conflicts() -> list[dict]:
    """Same term + same plane_scope + different block_hash."""
    conflicts = []
    rows = db.fetchall(
        """
...
def _detect_canon_collisions() -> list[dict]:
    ...
    If both plane_scope are empty, treat as global → collision allowed if topics overlap.
    """
    conflicts = []
...
def _detect_planar_conflicts() -> list[dict]:
    ...
    conflicts = []
    now = int(time.time())
    cutoff = now - 86400  # C1: use cutoff = now - 86400

    # C1: Filter with ts >= cutoff directly in SQL
    rows = db.fetchall(
        """
        SELECT plane_path,
...
def list_conflicts() -> list[dict]:
    return db.fetchall("SELECT * FROM conflicts ORDER BY detected_ts DESC")
