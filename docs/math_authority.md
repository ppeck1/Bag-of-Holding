...
| CANON variables | `test_canon_variables.py` | Exact/partial parity; boundary clamp; NaN protection |

---

## 8. CANON Policy Variable Formulas

**policy_id:** `canon_variables_policy_v0_1`
**Module:** `app/core/fold_metrics.py`
**Locked since:** 2026-06-02

### Status

These are **derived CANON policy variables** — deterministic projections computed from
existing scalar state and metric context. They are not directly observed database facts.
All outputs are clamped to `[0.0, 1.0]`. `scores_are_truth_values` is always `False`.

### Inputs

All inputs come from `FoldScalarState` (already computed by `compute_fold_scalar_state`)
and `FoldMetricContext` (DB-derived boundary object). No new database queries are required.
No LLM-derived values. No stochastic values.

### Formula Definitions

Let:
- `fresh` = `clamp(freshness_score)`
- `canon` = `clamp(canon_readiness)`
- `conf` = `clamp(resolution_confidence)`
- `conf_p` = `clamp(conflict_pressure)`
- `drift` = `clamp(drift_risk)`
- `auth` = `clamp(authority_score)`
- `miss` = `len(FoldMetricContext.missing_fields)`

#### 8.1 drift_rate

```
drift_rate = clamp( (1 - fresh)^1.5 × 0.7  +  (1 - canon) × 0.3 )
```

Measures the velocity proxy of epistemic decay. Increases as freshness drops and as canon
readiness decreases. Distinct from `drift_risk` (a snapshot level); `drift_rate` weights
freshness decay non-linearly to amplify rapidly staling content.

#### 8.2 entropy

```
entropy = clamp( (1 - conf) × 0.6  +  min(miss / 6, 0.4) )
```

Measures epistemic uncertainty at node scale. Increases as resolution confidence decreases
and as more inputs are missing. Not Shannon entropy over a corpus distribution; a local
node-level uncertainty pressure.

#### 8.3 delta_c_star (Δc*)

```
delta_c_star = clamp( canon × (1 - conf_p) × (1 - drift) )
```

Measures the stability of canonical state. High when readiness is high, conflict is low,
and drift is low. Conceptual name: stability signal. Appropriate for inspector stability
indicators, confidence halo, or opacity — **not** for temporal axis position.

#### 8.4 gamma (Γ)

```
gamma = clamp( auth × (1 - conf_p) )
```

Effective authority after conflict damping. Distinct from raw `authority_score`. Use for
secondary damped-authority display (opacity, warning state, secondary badge). **Do not
replace raw authority tier with gamma** — the tier separation between source authority and
conflict state is intentional.

#### 8.5 omega_viability (ΩV) — LOCKED

```
omega_viability = clamp( authority
                         - norm(conflicts, 4) × 0.35
                         - norm(expired + uncertain, 6) × 0.20
                         - (1 - completeness) × 0.25 )

norm(v, denom)  = clamp(v / denom)
completeness    = present_count / 8  over the eight metadata fields
                  (title, project, document_class, canonical_layer,
                   authority_state, review_state, status, summary),
                  excluding values in {None, "", "unassigned", "unknown"}
authority       = authority_state.authority_score(status, canonical_layer, review_state)
conflicts / expired / uncertain = dcns.get_node_diagnostics counts
```

Viability margin — how far a node sits above the instability floor. **This is the same
formula as the graph projection's `viability_margin`** (`app/core/visual_projection.py`);
both call the neutral `app/core/scalar_metrics.omega_viability`. The fold-facing field name
is `omega_viability`; the graph keeps the legacy name `viability_margin`. Graph/Fold parity
is enforced by `tests/test_canon_geometry.py`.

#### 8.6 mismatch_gradient — PROVISIONAL

```
mismatch_gradient = clamp(1 - completeness)
```

Metadata incompleteness gradient. **This is the same formula as the graph projection's
`projection_loss`.** The mapping of `mismatch_gradient` to the governing CANON spec name is
**PROVISIONAL** — the governing spec for `mismatch_gradient` is not yet frozen. Until it is,
`mismatch_gradient` is defined operationally as `projection_loss` (metadata completeness
inverse) and may be re-specified by a future locked entry. Do not treat this mapping as final.

### Computation Modes

| Mode | When | Field availability |
|---|---|---|
| `exact` | Full `FoldScalarState` + `FoldMetricContext` + geometry (doc row + DCNS diagnostics) | All six fields populated |
| `exact` (no geometry) | Geometry not supplied to `compute_canon_variables` | `omega_viability` / `mismatch_gradient` = null with warnings; other four populated |
| `partial` | Batch path (`/api/fold/library`) | `entropy`, `delta_c_star`, `omega_viability`, `mismatch_gradient` = null with warnings |

### Implementation

Single formula location: `_cv_drift_rate`, `_cv_entropy`, `_cv_delta_c_star`, `_cv_gamma`
in `fold_metrics.py`; `omega_viability`, `mismatch_gradient`, `metadata_completeness`,
`viability_completeness_record` in the neutral `app/core/scalar_metrics.py`. Both
`compute_canon_variables` (exact) and `compute_canon_variables_partial` (batch) call these
helpers — no formula duplication. `visual_projection.py` delegates to the same neutral
module so the graph and fold paths cannot drift.

### Properties

- No formula may be changed without a new locked entry in this Change Log.
- `calculation_mode` must always be present in the wire response.
- `warnings[]` must be non-empty when any field is `null`.
- `policy_id` must always be `"canon_variables_policy_v0_1"` for this version.
- Out-of-range inputs are pre-clamped; `float('nan')` is handled by `_clamp`'s NaN guard.
