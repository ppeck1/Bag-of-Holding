...
| `boh_v2_phase_a` | `localStorage` | `/v2` app shell settings (density, mode, landing). Parsed in `app/ui2/js/app.js` `loadSettings()`. | Yes |
...
| `selection` | `null` | Inspector selection: `null`, `{ type: "metric", ns, count, delta }`, `{ type: "doc", doc }`, or `{ type: "card", card }` |
...
`state.scope` was **removed** (2026-06-09, commit `15219ef`). The prototype global "Scope: Project Atlas" selector and its fixture tree (`FX.scopeTree`) are gone; the TopBar shows a static `Library: Bag of Holding` chip plus the `Planes:` filter. Internal/backend scope concepts (authority, retrieval, aggregation) are unaffected — this was a frontend shell simplification only.
...
- `inspectorOpen`, `inspectorWidth`, and `visiblePlanes` are session-only (in-memory); they reset to defaults on page load.
...
| `fold_router` | `/api/fold` | Current Fold View: CurrentFoldPacket resolver (`GET /api/fold/node/{doc_id}`), full-trace stub (`GET /api/fold/node/{doc_id}/trace`, `available:false`), and batch scatter summary (`GET /api/fold/library`). | Read-only; no operator token required. |
...
- `target_id` uses the deterministic `"{axis}:{value}"` form (e.g. `project:boh`, `plane:authority`). Built by `_build_node_scale_actions(base_packet)` from packet axis values only (no DB access).
- Node-scale axes: `project` (allowed when `docs.project` present), `plane` (allowed when a PlaneCard or `canonical_layer` resolves), `domain` (always `allowed: false` at node scale — deliberate missing-axis case, resolution deferred to Phase 7). The cluster endpoints that would consume `target_id` are Phase 7b (deferred).
...
Deferred (Phases 7b-8):
- Cluster/corpus routes (7b), cluster UI (7c), 2.5D/3D Spatial Lab placeholder (8), full trace endpoint depth.
- Full trace stub returns `available: false`; compact trace is in the main packet.
- Phase 7+ design — aggregation engine implemented (7a, see above); cluster/corpus routes and UI (7b/7c) still deferred. Multi-axis cluster identity over `project`/`plane`/`domain` with `scope_id = "{axis}:{value}"`; dual-channel aggregation (worst-case label precedence + authority-weighted numeric mean, equal-weight fallback). See `docs/current_fold_view_phased_variable_map.md` and `docs/specs/boh_current_fold_view_v0_3_phase6plus_cluster_aggregation_buildspec.md`.
...
| Key | Storage | Purpose |
| --- | --- | --- |
| `boh_v2_phase_a` | `localStorage` | Settings persisted by the new governed UI (density, mode, landing, diagnostics). |
...
| `#fold` | Fold Workspace | `/api/fold/library`, `/api/graph/projection`, `/api/fold/node/{id}` |
| `#library` | Library | `/api/docs`, `/api/search`, `/api/planes` |
