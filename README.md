...
| **Settings** | 6 groups: General, Library & Indexing, Intake Automation, AI & Analysis, Visualization, Security & Advanced; BROWSER LOCAL (editable), SESSION LOCAL (token), READ-ONLY ENV (env-backed), NEEDS SETTINGS API (not yet connected) |
| **Status** | Runtime cells, scheduler/watcher section, Auto-Index panel (real status with live rebuild button, result display, polling), maintenance actions |
...
Shared right-side panel across all major screens (Current State, Library, Review Center, Authority & Audit, Capture & Intake).

**Selection types and display branches:**
- **Document** — title, project, status, authority, summary, definition count, event count. Enriched lazily from `GET /api/docs/{id}` with stale-selection guard.
- **PlaneCard** — plane, card type, topic, b/d/m, valid until, backing doc.
- **Metric** — currentness namespace metadata, delta.
- **Intake Capability** — source_ref, lifecycle, safety_lane, preservable, queryable.
- **Quarantine Record** — quarantine_record_id, category, reason, safety_lane.
- **Duplicate Pair** — doc IDs with paths, relationship, detected date.
- **Audit Event** — event_type, actor, timestamp, document link.
- **Review Conflict** — conflict type, term/path, acknowledged, detected date.
- **Review Proposal** — queue_id, doc_id, summary, created date.
- **Review Approval** — approval_id, target, action, required authority, queued date.
- **Review Queue Item** — queue_id, doc, reason, status, created date.
- **Authority Integrity** — node_id (drift risk) or target_id (violation), state/drift/risk, actor, timestamp.
- **Authority Ledger** — target, old/new authority (promotions) or actor/result (resolutions), reason, timestamp.
- **Authority Gate Result** — gate_id, operation, posture, actor, query.
- **Residence** — original_ref, current_ref, status, reason.

**Controls:**
- **Collapse** `‹` — dismisses panel, content reclaims full width.
- **Toggle** — click same row twice to close selection.
...
- **Bulk/browser import idempotency**: repeated upload of the same filename/content/target is skipped as unchanged without creating duplicate docs; same filename with changed content still imports to a collision-safe path.
...
The Fold Workspace renders six resolver-backed projections over project clusters. Web preserves topology, Risk Map plots risk vs readiness gap, Authority Path uses authority/state lanes, Evidence State uses plane/evidence lanes, Currentness Map plots authority vs freshness, and Timeline uses temporal position; expanded clusters reveal member nodes at their measured projection coordinates, with a toolkit for label density, edge density, fold axis, fit/reset, Cluster Focus, and Action Queue review.
...
GET  /api/fold/cluster/{axis}/{value}     Read-only aggregate fold packet for supported cluster axes
GET  /api/fold/corpus/{axis}              Read-only corpus roll-up for a supported cluster axis
...
### Governance & Authority Routes

#### Approval & Review Routes

| Route | Purpose | Auth | Example |
|-------|---------|------|---------|
| `GET  /api/governance/approve/pending` | List pending approval requests | operator-gated | `curl http://127.0.0.1:8000/api/governance/approve/pending` |
| `POST /api/governance/approve/request-supersede` | Request approval to supersede a canonical document | operator-gated | See request model below |
| `POST /api/governance/approve/request-patch` | Request approval to apply a review artifact patch | operator-gated | See request model below |
| `POST /api/governance/approve/request-edge` | Request approval to promote a suggested edge to governed authority | operator-gated | See request model below |
| `POST /api/governance/approve/{approval_id}/approve` | Execute an approval (creates immutable provenance artifact) | operator-gated | See decision model below |
| `POST /api/governance/approve/{approval_id}/reject` | Reject an approval request (no state change) | operator-gated | See decision model below |
| `GET  /api/governance/approve/{approval_id}/review-diff` | Get line-oriented review surface for LLM artifacts | operator-gated | `curl http://127.0.0.1:8000/api/governance/approve/{id}/review-diff` |
| `GET  /api/governance/approve/ledger` | Query approval ledger as a constitutional record | operator-gated | `curl 'http://127.0.0.1:8000/api/governance/approve/ledger?status=pending'` |
| `GET  /api/governance/approve/{doc_id}/provenance` | Get full signed provenance chain for all authority events on a document | public read | `curl http://127.0.0.1:8000/api/governance/approve/{doc_id}/provenance` |
| `GET  /api/governance/approve/{doc_id}/impact` | Preview documents that reference a target doc (before supersede) | public read | `curl http://127.0.0.1:8000/api/governance/approve/{doc_id}/impact` |
...
```json
{
  "current_doc_id": "doc-abc123",
  "replacement_doc_id": "doc-xyz789",
  "requested_by": "operator_id",
  "reason": "Updated canonical version with corrections",
  "affected_count": 42
}
```
...
{
  "doc_id": "doc-abc123",
  "patch_summary": "Fix grammatical errors in section 3.2",
  "diff_hash": "sha256-hash-of-proposed-content",
  "requested_by": "operator_id",
  "reason": "LLM review artifact flagged errors",
  "original_text": "original text snippet",
  "proposed_text": "corrected text snippet",
  "llm_source": "claude-3-sonnet-v0.1",
  "review_artifact": "artifact-id-or-path"
}
```
...
```json
{
  ...
  "edge_type": "derives_from|supersedes|contradicts|supports",
  "strength": 0.85,
  "requested_by": "operator_id",
  "reason": "Explicit governance of suggested relationship",
  "cross_project": false
}
```

*ReviewDecision (for approve/reject):*
```json
{
  "reviewed_by": "operator_id",
  "review_note": "Approved — content reviewed and verified accurate"
}
```
...
|------|---------|------------------|
| 200 | Success | Approval executed, provenance artifact created |
| 400 | Bad request | Invalid approval_id, malformed JSON, missing required fields |
| 401 | Unauthorized | No `X-BOH-Operator-Token` header (or token missing in dev-open mode fails) |
| 403 | Forbidden | Operator token rejected or insufficient permissions |
| 404 | Not found | approval_id does not exist |
| 409 | Conflict | Attempt to approve already-completed or rejected request |
| 422 | Unprocessable | `review_note` missing/empty, invalid `diff_hash`, affected doc not found |

#### Duplicate Decision Route

| Route | Purpose | Auth | Decision Values |
|-------|---------|------|-----------------|
| `GET  /api/duplicates` | List all content duplicate candidates (hash-detected) | public read | N/A |
| `POST /api/duplicates/decision` | Record a duplicate review decision (no file deletion) | operator-gated | `canonical`, `duplicate`, `ignored`, `quarantine` |

**DuplicateDecision Request Model:**
```json
{
  "doc_id": "doc-abc123",
  "related_doc_id": "doc-xyz789",
  "decision": "canonical",
  "note": "First version; related_doc is a mirror copy"
}
```

**Decision Values Explained:**

- `canonical`: Mark doc_id as canonical; related_doc remains available but not canonical
- `duplicate`: Mark both as duplicate content; neither gains canonical authority
- `ignored`: Mark duplicate relationship as a false positive; keep both as-is
- `quarantine`: Isolate both documents to quarantine layer (mutation-blocking; containment hold)

**Note:** Bag of Holding **never deletes source files**, even after a duplicate decision. Dispositions are review metadata only.

**Response Status Codes:**

| Code | Meaning | Example |
|------|---------|---------|
| 200 | Success | Decision recorded, audit ledger updated, quarantine applied if requested |
| 400 | Bad request | Invalid `decision` value (must be one of: canonical, duplicate, ignored, quarantine) |
| 401 | Unauthorized | Missing operator token in non-dev-open mode |
| 403 | Forbidden | Operator token invalid |
| 404 | Not found | doc_id or related_doc_id not found |
| 409 | Conflict | One or both docs are promoted intake documents (mutation-isolated); duplicate decision blocked |
| 422 | Unprocessable | Malformed JSON, missing required fields |

#### ICS Export Route

| Route | Purpose | Auth | Format |
|-------|---------|------|--------|
| `GET /api/events/export.ics` | Export events as ICS calendar format | public read | `text/plain` (iCalendar) |

**Query Parameters:**

| Param | Type | Default | Effect |
|-------|------|---------|--------|
| `doc_id` | string (optional) | none | If provided, export events for only that document; if omitted, export all system events |

**Example:**

```bash
# All system events
curl http://127.0.0.1:8000/api/events/export.ics -o calendar.ics

# Events for one document
curl 'http://127.0.0.1:8000/api/events/export.ics?doc_id=doc-abc123' -o doc_events.ics
```

**Use Case:** Import into calendar applications (Apple Calendar, Google Calendar, Outlook) to track document lifecycle events, approval milestones, and freshness windows.
...
| Code | Meaning |
|------|---------|
| 200 | Success; ICS stream returned |
| 404 | `doc_id` provided but document not found |
...
Runtime variable matrix: `docs/VARIABLE_MATRIX.md`
...
Current verified baseline (2026-07-07): `python -m pytest tests -q` -> `1259 passed, 3 warnings in 259.30s`.
...
| **Current Fold View 6** | **Additive read-only `scale_actions[]` on the node `CurrentFoldPacket` (project + plane roll-up affordances; domain allowed only when exactly one registered domain resolves, otherwise disallowed with a reason) · surfaces via `GET /api/fold/node/{doc_id}` · no mutation or DB writes** |
| **Current Fold View 7a** | **Pure read-only cluster/corpus aggregation engine (`app/core/fold_aggregation.py`) rolling node `CurrentFoldPacket`s up to `(axis, value)` clusters and an axis-wide corpus · dual-channel (worst-case label precedence + per-pressure authority-weighted numeric mean with equal-weight fallback) · `canon_eligible` never surfaced true at aggregate scale · engine-only: no schema, no migration, no DB writes · route/UI follow-up tracked below** |
| **Current Fold View 7b** | **Read-only cluster/corpus API routes for fold aggregation: `/api/fold/cluster/{axis}/{value}`, `/api/fold/corpus/{axis}`, and cluster trace stub; project/plane/domain axes are operational, with domain resolved from registered substrate domains plus existing doc/card topic tokens; batch remains diagnostic-only; no writes, auth change, schema, or migration** |
| **Current Fold View 7c** | **Fold Workspace project/domain cluster UI consumes aggregation routes: project and linked-domain fold nodes, member hiding while collapsed, cluster inspector with local distribution plus aggregate packet, double-click expand/collapse** |
| **Fold Workspace usability 0.1+** | **Distinct projection coordinate semantics, measured-coordinate cluster expansion, session-scoped view toolkit (`boh_fold_view_v1`) with project/domain fold axis, Cluster Focus, Action Queue, and focused static coverage; frontend/read-only only** |
